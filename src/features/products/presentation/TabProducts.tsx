...
export function TabProducts({ onBack, routeResourceId, routeSubpage, onRouteChange }: { onBack: () => void; routeResourceId?: string | null; routeSubpage?: string | null; onRouteChange?: (resourceId: string | null, subpage?: string | null) => void }) {
  ...
  const resetForm = () => ({ name: "", slug: "", sku: "", short_description: "", description: "", price: "", compare_at_price: "", cover_media_id: "", is_active: true, is_featured: false, category_id: "", brand_id: "", external_platform: "", external_product_id: "", external_url: "" });
  const openNew = () => { if (!canCreate) return; setForm(resetForm()); setEditItem(null); setDrawerOpen(true); };
  const openEdit = (p: any) => { if (!(canViewDetails && canEdit)) return; setForm({ name: p.name || "", slug: p.slug || "", sku: p.sku || "", short_description: p.short_description || "", description: p.description || "", price: p.price == null ? "" : String(p.price), compare_at_price: p.compare_at_price == null ? "" : String(p.compare_at_price), cover_media_id: p.cover_media_id || "", is_active: p.is_active ?? true, is_featured: p.is_featured ?? false, category_id: p.category_id || "", brand_id: p.brand_id || "", external_platform: p.external_platform || "", external_product_id: p.external_product_id || "", external_url: p.external_url || "" }); setEditItem(p); setDrawerOpen(true); };
  ...
  useEffect(() => {
    ...
    const item = products.find((entry: any) => entry.id === routeResourceId); if (item) openEdit(item);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveProduct(payload, editItem?.id, user?.id ?? null); closeEditor(); setToast({ msg: editItem ? "Produto atualizado!" : "Produto criado!", type: "success" }); await refresh();
  ...
  return <div className="space-y-5">
    ...
    {canViewTable && <AdminCard>
      <AdminCardToolbar><div className="relative max-w-xs flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5a6a82]" /><input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Buscar produtos..." className={cn(INPUT, "py-2 pl-9 text-xs")} /></div></AdminCardToolbar>
      {loading ? <LoadingState /> : filtered.length === 0 ? <EmptyState icon={Package} title={search ? "Nenhum resultado" : "Nenhum produto cadastrado"} message="Adicione produtos para exibi-los na loja." onAdd={canCreate ? openNewPage : undefined} addLabel="Novo produto" /> : <div className="overflow-x-auto"><table className="min-w-[700px]"><thead><tr><th className="text-left">Produto</th><th className="text-left">Categoria</th><th className="text-left">Preço</th><th className="text-left">Destaque</th><th className="text-left">Status</th><th className="text-right">Ações</th></tr></thead><tbody>{pagedProducts.map(p => <tr key={p.id}><td><div className="flex items-center gap-3"><ProductAdminThumb mediaId={p.cover_media_id} name={p.name} /><span className="font-bold text-[#0d1b2e]">{p.name}</span></div></td><td className="text-xs text-[#5a6a82]">{p.categories?.name || "—"}</td><td className="font-bold text-[#0d1b2e]">{p.price ? `R$ ${Number(p.price).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}` : "Consultar"}</td><td>{canToggleFeatured && <button type="button" className="cursor-default" onClick={() => toggleFeatured(p)} title={p.is_featured ? "Remover destaque" : "Destacar produto"}>{p.is_featured ? <Star size={15} className="fill-amber-400 text-amber-400" /> : <Star size={15} className="text-[#5a6a82]" />}</button>}</td><td><StatusBadge status={p.is_active ? "Ativo" : "Inativo"} /></td><td><div className="flex items-center justify-end gap-1">{canViewDetails && canEdit && <AdminIconButton ariaLabel="Editar produto" title="Editar" onClick={() => openEditPage(p)}><Edit2 size={15} /></AdminIconButton>}{canToggleActive && <AdminIconButton ariaLabel={p.is_active ? "Desativar produto" : "Ativar produto"} title={p.is_active ? "Desativar" : "Ativar"} onClick={() => toggleActive(p)}><CheckCircle size={15} /></AdminIconButton>}{canDelete && <AdminIconButton ariaLabel="Excluir produto" title="Excluir" variant="danger" onClick={() => setDelId(p.id)}><Trash2 size={15} /></AdminIconButton>}</div></td></tr>)}</tbody></table></div>}
    ...
    <AdminPage open={drawerOpen} onClose={closeEditor} breadcrumb="Produtos" title={editItem ? "Editar produto" : "Novo produto"} maxW="max-w-xl"><div className="space-y-4 p-5"><Section title="Informações Principais"><div className="space-y-4"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><FInput label="Nome do produto" value={form.name} required onChange={(e: any) => setForm({ ...form, name: e.target.value, slug: editItem ? form.slug : slugify(e.target.value) })} placeholder="Nome do produto" /></div><FInput label="Slug" value={form.slug} onChange={(e: any) => setForm({ ...form, slug: e.target.value })} placeholder="slug-do-produto" /><FSelect label="Categoria" value={form.category_id} onChange={(e: any) => setForm({ ...form, category_id: e.target.value })} options={catOptions} /></div><FTextarea label="Descrição" value={form.description} onChange={(e: any) => setForm({ ...form, description: e.target.value })} rows={3} placeholder="Descreva o produto..." /></div></Section><Section title="Preço e Imagem"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FCurrencyInput label="Preço (R$)" value={form.price} onChange={(e: any) => setForm({ ...form, price: e.target.value })} placeholder="Deixe em branco para consultar" hint="Vazio = 'Consultar preço'" /><FCurrencyInput label="Preço de comparação (R$)" value={form.compare_at_price} onChange={(e: any) => setForm({ ...form, compare_at_price: e.target.value })} /></div><ImageUpload bucket="product-images" currentMediaId={form.cover_media_id} onUpload={mediaId => setForm({ ...form, cover_media_id: mediaId })} canUpload={editItem ? canEdit : canCreate} label="Imagem do produto" /></Section><Section title="Publicação"><div className="space-y-4"><FToggle label="Produto ativo" checked={form.is_active} onChange={v => setForm({ ...form, is_active: v })} /><FToggle label="Destaque" description="Exibe na Home e em destaques da loja." checked={form.is_featured} onChange={v => setForm({ ...form, is_featured: v })} /></div></Section></div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-5 py-4"><BtnSecondary onClick={closeEditor}>Cancelar</BtnSecondary>{(editItem ? canEdit : canCreate) && <BtnPrimary onClick={handleSave} disabled={saving}>{saving ? "Salvando..." : "Salvar produto"}</BtnPrimary>}</div></AdminPage>
  </div>;
}
