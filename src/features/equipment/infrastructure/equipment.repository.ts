...
export async function saveTechnicalField(field: { id?: string; label: string; field_key?: string; field_type: "text" | "number"; is_active: boolean; sort_order: number; }) {
  const organizationId = await getActiveOrganizationId();
  const payload = {
    ...(field.id ? {} : { field_key: field.field_key, organization_id: organizationId }),
  ...
  const query = field.id
    ? supabase.from("technical_fields").update(payload).eq("id", field.id).eq("organization_id", organizationId)
...
export async function saveEquipmentTypeTechnicalFields(equipmentTypeId: string, links: Array<{ technical_field_id: string; required: boolean; sort_order: number }>) {
  ...
  const result = await supabase.from("equipment_type_technical_fields").upsert(
    links.map(link => ({ organization_id: organizationId, equipment_type_id: equipmentTypeId, ...link })),
...
export async function saveServiceOrderTechnicalValues(serviceOrderId: string, values: Array<{ technical_field_id: string; field_key_snapshot: string; label_snapshot: string; field_type_snapshot: "text" | "number"; value_text?: string | null; value_number?: number | null; }>) {
  ...
  const organizationId = await getActiveOrganizationId();
  const result = await supabase.from("service_order_technical_values").upsert(
    values.map(value => ({ organization_id: organizationId, service_order_id: serviceOrderId, ...value })),
...
async function saveAndGetId(
  ...
  const query = id
    ? supabase.from(table).update(payload).eq("id", id).eq("organization_id", organizationId)
    : supabase.from(table).insert({ ...payload, organization_id: organizationId });
...
export async function saveEquipmentHierarchy(drafts: EquipmentDraft[], catalog: EquipmentCatalog): Promise<void> {
  ...
  for (const type of drafts) {
    ...
    const typeId = await saveAndGetId(
      "equipment_types",
      { name: type.name.trim(), slug: typeSlug, is_active: type.is_active, sort_order: 0 },
      type.id,
      organizationId,
    ...
    for (const brand of type.brands) {
      ...
      const brandId = await saveAndGetId(
        "equipment_brands",
        { name: brand.name.trim(), slug: brandSlug, equipment_type_id: typeId, is_active: brand.is_active, sort_order: 0 },
        brand.id,
        organizationId,
      ...
      for (const model of brand.models) {
        ...
        const { error } = model.id
          ? await supabase.from("equipment_models").update(payload).eq("id", model.id).eq("organization_id", organizationId)
          : await supabase.from("equipment_models").insert({ ...payload, organization_id: organizationId });
        if (error) throw error;
      }
