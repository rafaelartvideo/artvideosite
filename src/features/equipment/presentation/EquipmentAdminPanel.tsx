...
import { AlertCircle, CheckCircle, Edit2, Plus, Trash2, Wrench } from "lucide-react";
...
import { loadEquipmentCatalog, saveEquipmentHierarchy, saveTechnicalField } from "../infrastructure/equipment.repository";
...
export function EquipmentAdminPanel({ onBack, routeResourceId, routeSubpage, onRouteChange }: EquipmentAdminPanelProps) {
  ...
  const isSharedFromArtVideo = (item: { source_artvideo_id?: string | null }) => Boolean(item.source_artvideo_id);
  ...
  const openEdit = (type: EquipmentTypeRow) => { if (!(canViewDetails && canEdit) || isSharedFromArtVideo(type)) return; setDrafts([makeDraft(type)]); setFieldFormOpen(false); setActiveArea("registered"); setFormOpen(true); };
  ...
  const openEditField = (field: TechnicalField) => { if (!canManageFields || isSharedFromArtVideo(field)) return; setEditingField(field); setFieldForm({ label: field.label, field_key: field.field_key, field_type: field.field_type, is_active: field.is_active }); setFormOpen(false); setActiveArea("fields"); setFieldFormOpen(true); };
  ...
  const openEditPage = (type: EquipmentTypeRow) => canViewDetails && canEdit && !isSharedFromArtVideo(type) && (onRouteChange ? onRouteChange(type.id, "edit") : openEdit(type));
  ...
  const openEditFieldPage = (field: TechnicalField) => canManageFields && !isSharedFromArtVideo(field) && (onRouteChange ? onRouteChange(field.id, "field") : openEditField(field));
  ...
  const toggleField = async (field: TechnicalField) => {
    if (!canManageFields || isSharedFromArtVideo(field)) return;
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <div className="flex gap-1 border-b border-[#0d1b2e]/10">{canViewTable && <button type="button" onClick={openRegisteredPage} className={`cursor-default border-b-2 px-4 py-2.5 text-xs font-bold ${activeArea === "registered" ? "border-[#0057e7] text-[#0057e7]" : "border-transparent text-[#5a6a82]"}`}>Cadastrados</button>}{canViewFields && <button type="button" onClick={openFieldsPage} className={`cursor-default border-b-2 px-4 py-2.5 text-xs font-bold ${activeArea === "fields" ? "border-[#0057e7] text-[#0057e7]" : "border-transparent text-[#5a6a82]"}`}>Campos</button>}</div>
    {activeArea === "registered" && canViewTable ? <AdminCard>{loading ? <LoadingState /> : types.length === 0 ? <EmptyState icon={Wrench} title="Nenhum equipamento cadastrado" message="Cadastre o primeiro equipamento com suas marcas e modelos." onAdd={canCreate ? openNewPage : undefined} addLabel="Novo equipamento" /> : <><div className="overflow-x-auto"><table className="min-w-[680px]"><thead><tr>{showEquipment && <th className="text-left">Equipamento</th>}{showBrands && <th className="text-left">Marcas</th>}{showModels && <th className="text-left">Modelos</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedTypes.map(type => { const typeBrands = brands.filter(brand => brand.equipment_type_id === type.id); const brandIds = new Set(typeBrands.map(brand => brand.id)); const modelCount = models.filter(model => brandIds.has(model.equipment_brand_id)).length; return <tr key={type.id}>{showEquipment && <td className="font-bold text-[#0d1b2e]"><div className="flex flex-wrap items-center gap-2"><span>{type.name}</span>{isSharedFromArtVideo(type) && <span className="rounded-full border border-[#0057e7]/20 bg-[#0057e7]/5 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[#0057e7]">ArtVideo</span>}</div></td>}{showBrands && <td className="text-xs text-[#5a6a82]">{typeBrands.length}</td>}{showModels && <td className="text-xs text-[#5a6a82]">{modelCount}</td>}{showStatus && <td><StatusBadge status={type.is_active ? "Ativo" : "Inativo"} /></td>}{showActions && <td><div className="flex justify-end gap-1">{canViewDetails && canEdit && !isSharedFromArtVideo(type) && <AdminIconButton ariaLabel="Editar equipamento" title="Editar equipamento" onClick={() => openEditPage(type)}><Edit2 size={14} /></AdminIconButton>}</div></td>}</tr>; })}</tbody></table></div><PaginationBar page={safeTypePage} pageSize={typePageSize} totalItems={types.length} onPageChange={setTypePage} onPageSizeChange={size => { setTypePageSize(size); setTypePage(1); }} /></>}</AdminCard> : activeArea === "fields" && canViewFields ? <AdminCard><AdminCardHeader className="justify-end">{canManageFields && <AdminButton onClick={openNewFieldPage} className="text-xs"><Plus size={15} /> Novo campo</AdminButton>}</AdminCardHeader>{loading ? <LoadingState /> : technicalFields.length === 0 ? <EmptyState icon={Wrench} title="Nenhum campo técnico cadastrado" message="Cadastre o primeiro campo técnico." onAdd={canManageFields ? openNewFieldPage : undefined} addLabel="Novo campo" /> : <><div className="overflow-x-auto"><table className="min-w-[680px]"><thead><tr>{showFieldName && <th className="text-left">Campo</th>}{showFieldKey && <th className="text-left">Chave</th>}{showFieldType && <th className="text-left">Tipo</th>}{showFieldStatus && <th className="text-left">Status</th>}{showFieldActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedFields.map(field => <tr key={field.id}>{showFieldName && <td className="font-bold text-[#0d1b2e]"><div className="flex flex-wrap items-center gap-2"><span>{field.label}</span>{isSharedFromArtVideo(field) && <span className="rounded-full border border-[#0057e7]/20 bg-[#0057e7]/5 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-[#0057e7]">ArtVideo</span>}</div></td>}{showFieldKey && <td className="font-mono text-xs text-[#5a6a82]">{field.field_key}</td>}{showFieldType && <td className="text-xs text-[#5a6a82]">{field.field_type === "number" ? "Número" : "Texto"}</td>}{showFieldStatus && <td><StatusBadge status={field.is_active ? "Ativo" : "Inativo"} /></td>}{showFieldActions && <td><div className="flex justify-end gap-1">{canManageFields && !isSharedFromArtVideo(field) && <><AdminIconButton ariaLabel="Editar campo" title="Editar campo" onClick={() => openEditFieldPage(field)}><Edit2 size={14} /></AdminIconButton><AdminIconButton ariaLabel={field.is_active ? "Inativar campo" : "Ativar campo"} title={field.is_active ? "Inativar campo" : "Ativar campo"} onClick={() => void toggleField(field)}>{field.is_active ? <CheckCircle size={14} /> : <AlertCircle size={14} />}</AdminIconButton></>}</div></td>}</tr>)}</tbody></table></div><PaginationBar page={safeFieldPage} pageSize={fieldPageSize} totalItems={technicalFields.length} onPageChange={setFieldPage} onPageSizeChange={size => { setFieldPageSize(size); setFieldPage(1); }} /></>}</AdminCard> : null}</>}
    {editorRouteActive && !formOpen && !fieldFormOpen && <AdminCard className="p-8"><LoadingState /></AdminCard>}
    <AdminPage open={formOpen} onClose={closeEditor} breadcrumb="Equipamentos Técnicos" title="Cadastro de equipamentos" subtitle="Monte equipamento, marcas e modelos antes de salvar"><div className="space-y-5 p-4 sm:p-5">{drafts.map((type, ti) => <div key={`${type.id || "new"}-${ti}`} className="space-y-4"><Section title="Equipamento"><div className="flex items-end gap-2"><div className="min-w-0 flex-1"><FInput label="Nome do equipamento" required value={type.name} onChange={(e: any) => updateDraft(ti, { name: e.target.value })} placeholder="Ex: Televisão" /></div>{drafts.length > 1 && canDelete && <button type="button" onClick={() => setDrafts(current => current.filter((_, i) => i !== ti))} className="cursor-default rounded-lg p-2.5 text-red-500 hover:bg-red-50"><Trash2 size={16} /></button>}</div></Section>{canViewFields && <Section title="Campos do equipamento"><div className="grid gap-3 sm:grid-cols-2">{technicalFields.filter(field => field.is_active || type.technicalFields.some(link => link.technical_field_id === field.id)).map(field => { const link = type.technicalFields.find(item => item.technical_field_id === field.id); return <AdminCard key={field.id} className="px-3 py-3 shadow-none"><label className="flex cursor-default items-start gap-2 text-sm text-[#0d1b2e]"><Checkbox disabled={!canManageFields} checked={Boolean(link)} onCheckedChange={checked => updateTechnicalField(ti, field.id, checked === true)} /><span className="min-w-0 flex-1 break-words font-semibold">{field.label}</span></label>{link && <label className="mt-2 flex cursor-default items-center gap-2 pl-6 text-xs text-[#5a6a82]"><Checkbox disabled={!canManageFields} checked={link.required} onCheckedChange={checked => updateTechnicalFieldRequired(ti, field.id, checked === true)} />Obrigatório</label>}</AdminCard>; })}</div></Section>}{catalog && <EquipmentChecklistSection draft={type} catalog={catalog} onChange={patch => updateDraft(ti, patch)} />}<Section title="Marcas e modelos"><div className="flex items-center justify-end">{canCreate && <AdminButton onClick={() => addBrand(ti)} className="text-xs"><Plus size={14} /> Nova marca</AdminButton>}</div><div className="mt-3 space-y-3">{type.brands.map((brand, bi) => <AdminCard key={`${brand.id || "new-brand"}-${bi}`} className="bg-[#f8fafc] p-4 shadow-none"><div className="flex items-end gap-2"><div className="min-w-0 flex-1"><FInput label="Marca" required value={brand.name} onChange={(e: any) => updateBrand(ti, bi, { name: e.target.value })} /></div>{canDelete && <button type="button" onClick={() => removeBrand(ti, bi)} className="cursor-default rounded-lg p-2.5 text-red-500"><Trash2 size={15} /></button>}</div><div className="mt-3 space-y-2 border-l-2 border-[#0057e7]/20 pl-3"><div className="flex items-center justify-between"><span className="text-[10px] font-bold uppercase text-[#5a6a82]">Modelos</span>{canCreate && <AdminButton onClick={() => addModel(ti, bi)} className="text-xs"><Plus size={14} /> Novo modelo</AdminButton>}</div>{brand.models.map((model, mi) => <div key={`${model.id || "new-model"}-${mi}`} className="flex items-center gap-2"><div className="min-w-0 flex-1"><FInput value={model.name} required onChange={(e: any) => updateModel(ti, bi, mi, { name: e.target.value })} /></div>{canDelete && <button type="button" onClick={() => removeModel(ti, bi, mi)} className="cursor-default rounded-lg p-2 text-red-500"><Trash2 size={14} /></button>}</div>)}</div></AdminCard>)}</div></Section></div>)}{canCreate && drafts.every(type => !type.id) && <BtnSecondary onClick={addEquipment}><Plus size={15} /> Adicionar equipamento</BtnSecondary>}</div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closeEditor}>Cancelar</BtnSecondary><BtnPrimary onClick={save} disabled={saving}>{saving ? "Salvando..." : "Salvar estrutura"}</BtnPrimary></div></AdminPage>
    <AdminPage open={fieldFormOpen && canManageFields} onClose={closeEditor} breadcrumb="Equipamentos Técnicos > Campos" title={editingField ? "Editar campo técnico" : "Novo campo técnico"} subtitle="Configure um campo reutilizável nos tipos de equipamento"><div className="space-y-5 p-4 sm:p-5"><Section title="Campo técnico"><div className="grid gap-4"><FInput label="Nome do campo" required value={fieldForm.label} onChange={(e: any) => setFieldForm(current => ({ ...current, label: e.target.value, ...(!editingField && !current.field_key ? { field_key: slugify(e.target.value).replace(/-/g, "_") } : {}) }))} /><FInput label="Chave" required disabled={Boolean(editingField)} value={fieldForm.field_key} onChange={(e: any) => setFieldForm(current => ({ ...current, field_key: e.target.value }))} /><FSelect label="Tipo" value={fieldForm.field_type} onChange={(e: any) => setFieldForm(current => ({ ...current, field_type: e.target.value }))} options={[{ value: "text", label: "Texto" }, { value: "number", label: "Número" }]} /><FToggle label="Campo ativo" checked={fieldForm.is_active} onChange={is_active => setFieldForm(current => ({ ...current, is_active }))} /></div></Section></div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closeEditor}>Cancelar</BtnSecondary><BtnPrimary onClick={() => void saveField()} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</BtnPrimary></div></AdminPage>
  </div>;
}
