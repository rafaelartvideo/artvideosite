...
export function EquipmentAdminPanel({ onBack, routeResourceId, routeSubpage, onRouteChange }: EquipmentAdminPanelProps) {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    <AdminPage open={formOpen} onClose={closeEditor} breadcrumb="Equipamentos Técnicos" title="Cadastro de equipamentos" subtitle="Monte equipamento, marcas e modelos antes de salvar"><div className="space-y-5 p-4 sm:p-5">{drafts.map((type, ti) => <div key={`${type.id || "new"}-${ti}`} className="space-y-4"><Section title="Equipamento"><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><FInput label="Nome do equipamento" required value={type.name} onChange={(e: any) => updateDraft(ti, { name: e.target.value })} placeholder="Ex: Televisão" /><div className="flex items-center gap-2">{type.id && <FToggle label="Equipamento ativo" disabled={!canToggleActive} checked={type.is_active} onChange={is_active => updateDraft(ti, { is_active })} />}{drafts.length > 1 && canDelete && <button type="button" onClick={() => setDrafts(current => current.filter((_, i) => i !== ti))} className="cursor-default rounded-lg p-2.5 text-red-500 hover:bg-red-50"><Trash2 size={16} /></button>}</div></div></Section>{canViewFields && <Section title="Campos Extras"><div className="divide-y divide-[#0d1b2e]/8">{technicalFields.filter(field => field.is_active || type.technicalFields.some(link => link.technical_field_id === field.id)).map(field => { const link = type.technicalFields.find(item => item.technical_field_id === field.id); return <div key={field.id} className="py-3 first:pt-0 last:pb-0"><label className="flex cursor-default items-start gap-2 text-sm text-[#0d1b2e]"><Checkbox disabled={!canManageFields} checked={Boolean(link)} onCheckedChange={checked => updateTechnicalField(ti, field.id, checked === true)} /><span className="min-w-0 flex-1 break-words font-semibold">{field.label}</span></label>{link && <label className="mt-2 flex cursor-default items-center gap-2 pl-6 text-xs text-[#5a6a82]"><Checkbox disabled={!canManageFields} checked={link.required} onCheckedChange={checked => updateTechnicalFieldRequired(ti, field.id, checked === true)} />Obrigatório</label>}</div>; })}</div></Section>}{catalog && <EquipmentChecklistSection draft={type} catalog={catalog} onChange={patch => updateDraft(ti, patch)} />}<Section title="Marcas e modelos"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-relaxed text-[#5a6a82]">Cadastre cada marca e organize os modelos vinculados a ela.</p>{canCreate && <AdminButton onClick={() => addBrand(ti)} className="text-xs"><Plus size={14} /> Nova marca</AdminButton>}</div><div className="mt-4 divide-y divide-[#0d1b2e]/10">{type.brands.map((brand, bi) => <div key={`${brand.id || "new-brand"}-${bi}`} className="py-4 first:pt-0 last:pb-0"><div className="grid gap-4 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]"><div className="flex items-end gap-2"><div className="min-w-0 flex-1"><FInput label="Marca" required value={brand.name} onChange={(e: any) => updateBrand(ti, bi, { name: e.target.value })} /></div>{canDelete && <button type="button" aria-label="Excluir marca" title="Excluir marca" onClick={() => removeBrand(ti, bi)} className="cursor-default rounded-lg p-2.5 text-red-500 hover:bg-red-50"><Trash2 size={15} /></button>}</div><div className="min-w-0"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-wide text-[#5a6a82]">Modelos</span>{canCreate && <AdminButton onClick={() => addModel(ti, bi)} className="text-xs"><Plus size={14} /> Novo modelo</AdminButton>}</div><div className="mt-2 space-y-2">{brand.models.length === 0 && <p className="py-2 text-xs text-[#8a98aa]">Nenhum modelo cadastrado nesta marca.</p>}{brand.models.map((model, mi) => <div key={`${model.id || "new-model"}-${mi}`} className="flex items-center gap-2"><div className="min-w-0 flex-1"><FInput value={model.name} required placeholder="Nome do modelo" onChange={(e: any) => updateModel(ti, bi, mi, { name: e.target.value })} /></div>{canDelete && <button type="button" aria-label="Excluir modelo" title="Excluir modelo" onClick={() => removeModel(ti, bi, mi)} className="cursor-default rounded-lg p-2 text-red-500 hover:bg-red-50"><Trash2 size={14} /></button>}</div>)}</div></div></div></div>)}</div></Section></div>)}{canCreate && drafts.every(type => !type.id) && <BtnSecondary onClick={addEquipment}><Plus size={15} /> Adicionar equipamento</BtnSecondary>}</div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closeEditor}>Cancelar</BtnSecondary><BtnPrimary onClick={save} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</BtnPrimary></div></AdminPage>
