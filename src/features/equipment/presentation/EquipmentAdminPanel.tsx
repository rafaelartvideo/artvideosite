...
export function EquipmentAdminPanel({ onBack, routeResourceId, routeSubpage, onRouteChange }: EquipmentAdminPanelProps) {
  ...
  useEffect(() => {
    ...
    if (routeSubpage === "edit") { const type = types.find(item => item.id === routeResourceId); if (type && canViewDetails && canEdit && (!formOpen || drafts[0]?.id !== type.id)) openEdit(type); return; }
    if (routeSubpage === "field") { const field = technicalFields.find(item => item.id === routeResourceId); if (field && canManageFields && (!fieldFormOpen || editingField?.id !== field.id)) openEditField(field); }
