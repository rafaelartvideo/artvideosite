...
export function EquipmentAdminPanel({ onBack }: { onBack: () => void }) {
  ...
  const updateTechnicalField = (typeIndex: number, fieldId: string, checked: boolean) => setDrafts((current) => current.map((type, index) => {
    ...
    const technicalFields = checked ? existing ? type.technicalFields : [...type.technicalFields, { technical_field_id: fieldId, required: false, sort_order: type.technicalFields.length * 10 }] : type.technicalFields.filter((field) => field.technical_field_id !== fieldId);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 border-b border-[#0d1b2e]/10">
        <button type="button" onClick={() => setActiveArea("registered")} className={`px-4 py-2.5 text-xs font-bold border-b-2 ${activeArea === "registered" ? "border-[#0057e7] text-[#0057e7]" : "border-transparent text-[#5a6a82]"}`}>Cadastrados</button>
        <button type="button" onClick={() => setActiveArea("fields")} className={`px-4 py-2.5 text-xs font-bold border-b-2 ${activeArea === "fields" ? "border-[#0057e7] text-[#0057e7]" : "border-transparent text-[#5a6a82]"}`}>Campos</button>
      </div>
      {activeArea === "registered" ? (
        ...
        <AdminCard>
          ...
          {loading ? <LoadingState /> : technicalFields.length === 0 ? <EmptyState icon={Wrench} title="Nenhum campo técnico cadastrado" message="Cadastre o primeiro campo técnico." onAdd={hasPermission("equipment.create") ? openNewField : undefined} addLabel="Novo campo" /> : <div className="divide-y divide-[#0d1b2e]/5">{technicalFields.map((field) => <div key={field.id} className="px-5 py-3.5 flex items-center justify-between gap-3"><div><p className="font-bold text-[#0d1b2e]">{field.label}</p><p className="text-[11px] text-[#5a6a82] font-mono">{field.field_key} - {field.field_type === "number" ? "Número" : "Texto"}</p></div><div className="flex items-center gap-2"><StatusBadge status={field.is_active ? "Ativo" : "Inativo"} />{hasPermission("equipment.edit") && <><AdminIconButton ariaLabel="Editar campo" title="Editar campo" onClick={() => openEditField(field)}><Edit2 size={14} /></AdminIconButton><AdminIconButton ariaLabel={field.is_active ? "Inativar campo" : "Ativar campo"} title={field.is_active ? "Inativar campo" : "Ativar campo"} onClick={() => void toggleField(field)}>{field.is_active ? <CheckCircle size={14} /> : <AlertCircle size={14} />}</AdminIconButton></>}</div></div>)}</div>}
        </AdminCard>
      )}
      ...
      <AdminPage open={formOpen} onClose={() => setFormOpen(false)} breadcrumb="Equipamentos Técnicos" title="Cadastro de equipamentos" subtitle="Monte equipamento, marcas e modelos antes de salvar">
        <div className="p-5 space-y-5">
          {drafts.map((type, typeIndex) => <div key={`${type.id || "new"}-${typeIndex}`} className="space-y-4">
            <Section title="Equipamento"><div className="flex items-end gap-2"><div className="flex-1"><FInput label="Nome do equipamento" required value={type.name} onChange={(e: any) => updateDraft(typeIndex, { name: e.target.value })} placeholder="Ex: Televisão" /></div>{drafts.length > 1 && hasPermission("equipment.delete") && <button type="button" onClick={() => setDrafts((current) => current.filter((_, index) => index !== typeIndex))} className="p-2.5 text-red-500 hover:bg-red-50 rounded-lg" title="Remover equipamento"><Trash2 size={16} /></button>}</div></Section>
            <Section title="Campos do equipamento"><div className="grid sm:grid-cols-2 gap-3">{technicalFields.filter((field) => field.is_active || type.technicalFields.some((link) => link.technical_field_id === field.id)).map((field) => {
              const link = type.technicalFields.find((item) => item.technical_field_id === field.id);
              return <AdminCard key={field.id} className="px-3 py-2.5 shadow-none"><label className="flex cursor-pointer items-start gap-2 text-sm text-[#0d1b2e]"><Checkbox checked={Boolean(link)} onCheckedChange={(checked) => updateTechnicalField(typeIndex, field.id, checked === true)} /><span className="min-w-0 flex-1 font-semibold">{field.label}{!field.is_active && <span className="ml-2 text-[10px] font-bold text-amber-600">Inativo</span>}</span></label>{link && <label className="mt-2 flex items-center gap-2 pl-6 text-xs text-[#5a6a82]"><Checkbox checked={link.required} onCheckedChange={(checked) => updateTechnicalFieldRequired(typeIndex, field.id, checked === true)} />Obrigatório</label>}</AdminCard>;
            })}</div></Section>
        ...
        <div className="sticky bottom-0 bg-white border-t border-[#0d1b2e]/8 px-5 py-4 flex justify-end gap-3"><BtnSecondary onClick={() => setFormOpen(false)}>Cancelar</BtnSecondary>{drafts.length > 0 && <BtnPrimary onClick={save} disabled={saving}>{saving ? "Salvando..." : "Salvar estrutura"}</BtnPrimary>}</div>
      </AdminPage>
