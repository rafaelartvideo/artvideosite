...
export function FinanceEntriesSection({ entryType, selectedEntryId, onSelectEntry }: { entryType: FinancialEntryType; selectedEntryId?: string | null; onSelectEntry: (id: string | null) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(entry => {
      ...
      if (!term) return true;
      return [entry.description, entry.counterpart_name_snapshot, entry.counterpart_document_snapshot, entry.origin_reference].some(value => String(value || "").toLocaleLowerCase("pt-BR").includes(term));
    });
