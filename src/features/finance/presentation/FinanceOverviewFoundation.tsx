...
export function FinanceOverviewFoundation() {
  ...
  return <div className="space-y-4">
    <div>
      <h2 className="text-lg font-black text-[#0d1b2e]">Configuração financeira</h2>
      <p className="mt-1 text-xs leading-relaxed text-[#5a6a82]">Acompanhe a preparação da base do Financeiro. Saldos, contas a pagar/receber e relatórios entram nas próximas etapas.</p>
    </div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <ReadinessCard title="Centros de custo" value={costCenters.filter(item => item.is_active).length} detail="Centros de custo disponíveis para rateios futuros." icon={Target} />
      ...
      <ReadinessCard title="Limite de 2 aprovações" value={settings?.second_approval_threshold == null ? "Não definido" : settings.second_approval_threshold.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} detail="Aplicado ao Contas a Pagar quando o fluxo de aprovações for ativado." icon={Settings2} />
