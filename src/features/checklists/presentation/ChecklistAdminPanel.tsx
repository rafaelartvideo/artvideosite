...
const RESPONSE_OPTIONS = [
  { value: "conformity", label: "Conforme / Não conforme" }, { value: "yes_no", label: "Sim / Não" },
  { value: "confirmation", label: "Confirmação" }, { value: "text", label: "Texto" }, { value: "number", label: "Número" },
];
const PHOTO_OPTIONS = [
  { value: "none", label: "Sem foto" }, { value: "optional", label: "Foto opcional" },
  { value: "required", label: "Foto obrigatória" }, { value: "required_on_failure", label: "Obrigatória se houver falha" },
];
const OBSERVATION_OPTIONS = [
  { value: "none", label: "Sem observação" }, { value: "optional", label: "Observação opcional" },
  { value: "required", label: "Observação obrigatória" }, { value: "required_on_failure", label: "Obrigatória se houver falha" },
];
const STAGE_TYPE_OPTIONS = [
  { value: "entry", label: "Entrada" }, { value: "diagnosis", label: "Diagnóstico" },
  { value: "qc", label: "Saída / QC" }, { value: "custom", label: "Personalizada" },
];
...
function newStage(name: string, stage_type: ChecklistStageType, code: string, sort_order: number): ChecklistProfileDraftStage {
  return { code, stage_type, name, situation_id: null, block_situation_exit: false, block_resolution: stage_type === "diagnosis", block_completion: stage_type === "qc", sort_order, is_active: true, items: [newItem()] };
}
function newDraft(): Draft {
  return { name: "", description: "", is_active: true, stages: [newStage("Entrada", "entry", "entrada", 0), newStage("Diagnóstico", "diagnosis", "diagnostico", 10), newStage("Saída / QC", "qc", "saida_qc", 20)] };
}
...
export function ChecklistAdminPanel({ onBack, routeResourceId, routeSubpage, onRouteChange }: Props) {
  ...
  const moveStage = (stageIndex: number, direction: -1 | 1) => setDraft(current => {
    ...
    const stages = [...current.stages]; [stages[stageIndex], stages[target]] = [stages[target], stages[stageIndex]];
  ...
  const toggleProfile = async (profileId: string) => {
    ...
    const editor = profileEditorFromAdminData(data, profileId); if (!editor) return;
        ...
        <section className="overflow-hidden rounded-2xl border border-[#0d1b2e]/10 bg-white">
          <div className="flex flex-col gap-3 border-b border-[#0d1b2e]/8 bg-[#f8fafc] p-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-[#0057e7]/15 bg-[#eef5ff] text-[#0057e7]"><ClipboardCheck size={18} /></div>
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-black text-[#0d1b2e]">Etapas do checklist</h3><span className="rounded-full border border-[#0057e7]/15 bg-[#eaf2ff] px-2.5 py-0.5 text-[10px] font-black text-[#0057e7]">{draft.stages.length} {draft.stages.length === 1 ? "etapa" : "etapas"}</span></div><p className="mt-1 text-xs leading-5 text-[#6b7c93]">Organize o fluxo, vincule situações da OS e defina os bloqueios de cada etapa.</p></div>
            </div>
            <AdminButton className="w-full justify-center sm:w-auto" onClick={addStage}><Plus size={17} /> Nova etapa</AdminButton>
          </div>
