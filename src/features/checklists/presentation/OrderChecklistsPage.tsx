...
export function OrderChecklistsPage({ open, order, canManage, canReopen, onClose }: {
  open: boolean;
  order: any;
  canManage: boolean;
  canReopen: boolean;
  ...
  const completeStage = async (stage: OrderChecklistStage) => {
    ...
    try {
      ...
      await refresh();
      setToast({ msg: `Etapa ${stage.name_snapshot} concluída.`, type: "success" });
    } catch (error) {
  ...
  return <AdminPage open onClose={onClose} breadcrumb={`Ordens de Serviço > ${order?.os_number || "OS"} > Checklists`} title="Checklists da OS" subtitle={checklist ? `${checklist.profile_name_snapshot} • versão ${checklist.profile_version_snapshot}` : "Checklist técnico do equipamento"} maxW="max-w-4xl">
    ...
    <div className="space-y-4 p-5">
      {query.isPending ? <LoadingState text="Preparando checklist da OS..." /> : query.error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{query.error instanceof Error ? query.error.message : String(query.error)}</div> : !checklist ? <EmptyState icon={ClipboardCheck} title="Este equipamento não possui checklist" message="Vincule um Perfil de Checklist ao tipo de equipamento em Operação > Equipamentos." /> : <>
        <div className="rounded-xl border border-[#0057e7]/15 bg-[#eef5ff]/60 p-4">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-[#5b6d86]">Progresso geral</p><p className="mt-1 text-xl font-black text-[#0d1b2e]">{progress.answered}/{progress.total} itens respondidos</p></div><StatusBadge status={checklist.status === "completed" ? "Concluído" : checklist.status === "in_progress" ? "Em andamento" : "Pendente"} /></div>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[#0057e7] transition-all" style={{ width: `${progress.percentage}%` }} /></div>
        </div>
        {checklist.stages.map(stage => {
          const stageProgress = checklistStageProgress(stage);
          const current = Boolean(stage.situation_id_snapshot && stage.situation_id_snapshot === order?.situation_id);
          const completed = stage.status === "completed";
          return <AdminCard key={stage.id} className={current ? "ring-2 ring-[#0057e7]/20" : ""}>
            <AdminCardHeader title={stage.name_snapshot} subtitle={stage.situation_name_snapshot ? `Situação: ${stage.situation_name_snapshot}${current ? " • Atual" : ""}` : "Etapa manual"} actions={<StatusBadge status={completed ? "Concluído" : stage.status === "in_progress" || stage.status === "reopened" ? "Em andamento" : "Pendente"} />} />
            <div className="space-y-4 p-4">
              <div className="flex items-center justify-between text-xs text-[#5a6a82]"><span>{stageProgress.answered}/{stageProgress.total} respondidos</span><span>{stageProgress.percentage}%</span></div>
              <div className="h-1.5 overflow-hidden rounded-full bg-[#e8edf4]"><div className="h-full rounded-full bg-[#0057e7]" style={{ width: `${stageProgress.percentage}%` }} /></div>
              <div className="space-y-3">{stage.items.map((item, index) => <OrderChecklistItemEditor key={item.id} item={item} number={index + 1} checklistId={checklist.id} order={order} disabled={!canManage || completed} onChanged={refresh} onError={message => setToast({ msg: message, type: "error" })} onSuccess={message => setToast({ msg: message, type: "success" })} />)}</div>
              <div className="flex flex-wrap justify-end gap-2 border-t border-[#0d1b2e]/8 pt-3">
                {completed && canReopen && <AdminButton variant="secondary" disabled={busyStageId === stage.id} onClick={() => void reopenStage(stage)}><RotateCcw size={14} /> Reabrir etapa</AdminButton>}
                {!completed && canManage && <BtnPrimary disabled={busyStageId === stage.id} onClick={() => void completeStage(stage)}><CheckCircle2 size={15} /> {busyStageId === stage.id ? "Validando..." : "Concluir etapa"}</BtnPrimary>}
              </div>
            </div>
          </AdminCard>;
        })}
...
function OrderChecklistItemEditor({ item, number, checklistId, order, disabled, onChanged, onError, onSuccess }: {
...
function answerOptions(item: OrderChecklistItem) {
  ...
  return [{ value: "confirmed", label: "Confirmado" }];
}
