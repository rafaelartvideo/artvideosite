...
import { FInput, FIntegerInput, FSelect, FTextarea, FToggle, INPUT, FCurrencyInput } from "@/shared/ui/admin/AdminFormControls";
...
function ServiceDrawer({ open, onClose, editItem, categories, brands, products, userId, onToast }: { open: boolean; onClose: () => void; editItem: any | null; categories: any[]; brands: any[]; products: any[]; userId: string | null; onToast: (toast: { msg: string; type: "success" | "error" }) => void }) {
  ...
  const [name, setName] = useState(""); const [categoryId, setCategoryId] = useState(""); const [brandId, setBrandId] = useState(""); const [productId, setProductId] = useState(""); const [coverMediaId, setCoverMediaId] = useState(""); const [shortDesc, setShortDesc] = useState(""); const [description, setDescription] = useState(""); const [basePrice, setBasePrice] = useState("");
  ...
  useEffect(() => {
    ...
    if (editItem) {
      ...
      setName(""); setCategoryId(""); setBrandId(""); setProductId(""); setCoverMediaId(""); setShortDesc(""); setDescription(""); setBasePrice(""); setPriceMode("QUOTE"); setActive(true); setFeatured(false); setSortOrder(0); setVariants([]); setFeatures([]); setExclusions([]); setPriceFactors([]); setSections([]); setFaqs([]);
    }
  }, [open, editItem, availableTabs]);

  const handleSave = async () => {
    if (!canSave) return;
    if (canInfo && !name.trim()) { onToast({ msg: "Nome do serviço é obrigatório.", type: "error" }); return; }
    const parsedBasePrice = basePrice === "" ? null : Number(basePrice);
    if (canPrice && parsedBasePrice !== null && (!Number.isFinite(parsedBasePrice) || parsedBasePrice < 0)) { onToast({ msg: "Informe um preço base válido e não negativo.", type: "error" }); return; }
    if (canPrice && (priceMode === "FIXED" || priceMode === "STARTING_FROM") && parsedBasePrice === null) { onToast({ msg: "Informe o preço base para o modo de preço selecionado.", type: "error" }); return; }
    if (canVariants) {
      for (let index = 0; index < variants.length; index += 1) {
        const variant = variants[index];
        if (!variant.title.trim()) { onToast({ msg: `Informe o título da variação ${index + 1}.`, type: "error" }); return; }
        if (variant.price !== "") { const price = Number(variant.price); if (!Number.isFinite(price) || price < 0) { onToast({ msg: `Informe um preço válido na variação ${index + 1}.`, type: "error" }); return; } }
      }
    }
    if (canFactors) {
      for (let index = 0; index < priceFactors.length; index += 1) {
        const factor = priceFactors[index];
        if (!factor.name.trim()) { onToast({ msg: `Informe o título do fator ${index + 1}.`, type: "error" }); return; }
        if (factor.amount !== "") { const amount = Number(factor.amount); if (!Number.isFinite(amount) || amount < 0) { onToast({ msg: `Informe um valor válido no fator ${index + 1}.`, type: "error" }); return; } }
      }
    }
    if (canPublication && (!Number.isInteger(Number(sortOrder)) || Number(sortOrder) < 0)) { onToast({ msg: "A ordem de exibição deve ser um número inteiro não negativo.", type: "error" }); return; }
    setSaving(true);
    try {
      const payload: Record<string, unknown> = {};
      if (canInfo) {
        const finalSlug = await generateUniqueSlug("services", name, editItem?.id);
        Object.assign(payload, { title: name.trim(), slug: finalSlug, category_id: categoryId || null, brand_id: brandId || null, product_id: productId || null, short_description: shortDesc.trim() || null, description: description.trim() || null });
      }
      if (canMedia) payload.cover_media_id = coverMediaId || null;
      if (canPrice) Object.assign(payload, { base_price: parsedBasePrice, price_mode: priceMode });
      if (canPublication) Object.assign(payload, { is_featured: featured, sort_order: Number(sortOrder) });
  ...
  return <AdminPage open={open && canSave} onClose={onClose} breadcrumb="Serviços" title={editItem ? `Editar: ${editItem.title}` : "Novo serviço"} subtitle="As seções exibidas respeitam as permissões da função" maxW="max-w-3xl">
    ...
    <div className="space-y-4 p-5">
      ...
      {tab === "publish" && <Section title="Publicação"><div className="space-y-4">{canToggleActive && <FToggle label="Serviço ativo" description="Serviços inativos não aparecem no site." checked={active} onChange={setActive} />}{canPublication && <><FToggle label="Destaque" description="Exibe o serviço como destaque na Home e na listagem." checked={featured} onChange={setFeatured} /><FIntegerInput label="Ordem de exibição" value={String(sortOrder)} onChange={(event: any) => setSortOrder(Number(event.target.value || 0))} /></>}</div></Section>}
    </div>
    <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-[#0d1b2e]/8 bg-white px-6 py-4"><p className="text-xs text-[#5a6a82]">Somente as seções autorizadas serão alteradas.</p><div className="flex gap-3"><BtnSecondary onClick={onClose}>Cancelar</BtnSecondary>{canSave && <BtnPrimary onClick={handleSave} loading={saving} loadingText="Salvando...">Salvar</BtnPrimary>}</div></div>
  </AdminPage>;
}
