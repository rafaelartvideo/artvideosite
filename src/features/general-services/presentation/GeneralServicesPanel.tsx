...
function GeneralServicesPanelContent({ onBack, routeResourceId, routeSubpage, onRouteChange }: GeneralServicesPanelProps) {
  ...
  const openNew = () => { if (!canCreate) return; setEditItem(null); setName(""); setPrice(""); setMaxDiscountPercentage(""); setMaxDiscountAmount(""); setPriceAtCompletion(false); setActive(true); setFormOpen(true); };
  const openEdit = (item: GeneralService) => { if (!(canViewDetails && canEdit)) return; setEditItem(item); setName(item.name || ""); setPrice(item.price == null ? "" : String(item.price)); setMaxDiscountPercentage(item.max_discount_percentage == null ? "" : String(item.max_discount_percentage)); setMaxDiscountAmount(item.max_discount_amount == null ? "" : String(item.max_discount_amount)); setPriceAtCompletion(Boolean(item.price_at_completion)); setActive(item.is_active !== false); setFormOpen(true); };
  ...
  const save = async () => {
    ...
    const parsedPrice = priceAtCompletion ? null : price === "" ? null : Number(price);
    const parsedDiscount = maxDiscountPercentage === "" ? null : Number(maxDiscountPercentage);
    const parsedDiscountAmount = maxDiscountAmount === "" ? null : Number(maxDiscountAmount);

    if (parsedPrice !== null && (!Number.isFinite(parsedPrice) || parsedPrice < 0)) {
      setToast({ msg: "Informe um valor válido e não negativo.", type: "error" }); return;
    }
    ...
    if (parsedDiscountAmount !== null && (!Number.isFinite(parsedDiscountAmount) || parsedDiscountAmount < 0)) {
      setToast({ msg: "O desconto máximo em valor deve ser igual ou maior que zero.", type: "error" }); return;
    }
    ...
    try {
      const payload = {
        ...
        max_discount_amount: parsedDiscountAmount,
        is_active: active,
      };
  ...
  return <div className="min-w-0 space-y-5">{toast && <Toast message={toast.msg} type={toast.type} onClose={() => setToast(null)} />}{!routeResourceId && <><InternalBackButton onBack={onBack} /><PageHeader title="Serviços Gerais" subtitle="Serviços técnicos internos utilizados na operação" actions={canCreate ? <AdminButton onClick={openNewPage} className="text-xs"><Plus size={16} /> Novo serviço</AdminButton> : null} />
    {canViewTable && <AdminCard>{loading ? <LoadingState /> : items.length === 0 ? <EmptyState icon={Wrench} title="Nenhum serviço geral cadastrado" message="Cadastre um serviço técnico interno." onAdd={canCreate ? openNewPage : undefined} addLabel="Novo serviço" /> : <><div className="overflow-x-auto"><table className="min-w-[700px]"><thead><tr>{showService && <th className="text-left">Serviço</th>}{showPrice && <th className="text-left">Valor</th>}{showMaxDiscount && <th className="text-left">Desconto máximo</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedItems.map(item => <tr key={item.id}>{showService && <td className="font-bold text-[#0d1b2e]">{item.name}</td>}{showPrice && <td className="text-[#0d1b2e]">{item.price_at_completion ? <span className="font-semibold text-[#0057e7]">Na conclusão da OS</span> : formatCurrency(item.price, "Não informado")}</td>}{showMaxDiscount && <td className="text-[#5a6a82]"><div>{item.max_discount_percentage == null ? "Não informado" : `${formatNumber(item.max_discount_percentage, { maximumFractionDigits: 2 })}%`}</div><div className="mt-0.5 text-xs">{item.max_discount_amount == null ? "Não informado" : formatCurrency(item.max_discount_amount)}</div></td>}{showStatus && <td><StatusBadge status={item.is_active ? "Ativo" : "Inativo"} /></td>}{showActions && <td><div className="flex justify-end gap-1">{canViewDetails && canEdit && <AdminIconButton ariaLabel="Editar serviço geral" title="Editar" onClick={() => openEditPage(item)}><Edit2 size={15} /></AdminIconButton>}{canToggleActive && <AdminActiveStateButton active={item.is_active} entityLabel="serviço geral" onClick={() => void toggle(item)} />}</div></td>}</tr>)}</tbody></table></div><PaginationBar page={safePage} pageSize={pageSize} totalItems={items.length} onPageChange={setPage} onPageSizeChange={size => { setPageSize(size); setPage(1); }} /></>}</AdminCard>}</>}
    {routeResourceId && !formOpen && <AdminCard className="p-8"><LoadingState /></AdminCard>}
    <AdminPage open={formOpen} onClose={closeForm} breadcrumb="Operação > Serviços Gerais" title={editItem ? editItem.name : "Novo serviço"} subtitle="Cadastro de serviço técnico interno"><div className="p-4 sm:p-5"><Section title="Serviço geral"><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><FInput label="Nome do serviço" required disabled={saving} value={name} onChange={(e: any) => setName(e.target.value)} /></div><div className="sm:col-span-2"><FToggle label="Definir valor na conclusão da OS" disabled={saving} checked={priceAtCompletion} onChange={value => { setPriceAtCompletion(value); if (value) setPrice(""); }} /></div><FCurrencyInput label="Valor" disabled={saving || priceAtCompletion} value={priceAtCompletion ? "" : price} onChange={(e: any) => setPrice(e.target.value)} hint={priceAtCompletion ? "O valor será informado ao concluir a OS." : undefined} /><FDecimalInput label="Desconto máximo (%)" disabled={saving} value={maxDiscountPercentage} decimalPlaces={2} onChange={(e: any) => setMaxDiscountPercentage(e.target.value)} /><FCurrencyInput label="Desconto máximo (R$)" disabled={saving} value={maxDiscountAmount} onChange={(e: any) => setMaxDiscountAmount(e.target.value)} /></div><div className="mt-4"><FToggle label="Serviço ativo" disabled={saving} checked={active} onChange={setActive} /></div></Section></div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closeForm} disabled={saving}>Cancelar</BtnSecondary>{(editItem ? canEdit : canCreate) && <BtnPrimary onClick={save} loading={saving} loadingText="Salvando...">Salvar</BtnPrimary>}</div></AdminPage>
  </div>;
}
