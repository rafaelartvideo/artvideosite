...
function ServiceTypesAdminPanelContent() {
  ...
  return <div className="space-y-5">
    ...
    {canViewTable && <AdminCard>{loading ? <LoadingState /> : items.length === 0 ? <EmptyState icon={List} title="Nenhum tipo cadastrado" message="Crie tipos para disponibilizá-los na Nova OS." onAdd={canCreate ? openNew : undefined} addLabel="Novo tipo" /> : <><div className="overflow-x-auto"><table className="min-w-[820px]"><thead><tr>{showType && <th className="text-left">Tipo de atendimento</th>}{showDescription && <th className="text-left">Descrição</th>}{showForecast && <th className="text-left">Previsão</th>}{showSla && <th className="text-left">SLA</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedItems.map(item => { const summary = slaSummary(item); return <tr key={item.id}>{showType && <td><p className="font-bold text-[#0d1b2e]">{item.title}</p></td>}{showDescription && <td><p className="max-w-sm break-words text-xs leading-relaxed text-[#5a6a82]">{item.description || "Sem descrição"}</p></td>}{showForecast && <td className="text-xs text-[#5a6a82]">{item.forecast_days == null ? "Não informada" : `${item.forecast_days} dia(s)`}</td>}{showSla && <td className="text-xs text-[#5a6a82]">{summary.count ? `${summary.count} situações · ${summary.totalHours}h` : "Não configurado"}</td>}{showStatus && <td><StatusBadge status={item.is_active ? "Ativo" : "Inativo"} /></td>}{showActions && <td><div className="flex items-center justify-end gap-1">{canViewDetails && canEdit && <AdminIconButton ariaLabel="Editar tipo" title="Editar" onClick={() => openEdit(item)}><Edit2 size={14} /></AdminIconButton>}{canToggleActive && <AdminIconButton ariaLabel={item.is_active ? "Desativar tipo" : "Ativar tipo"} title={item.is_active ? "Desativar" : "Ativar"} onClick={() => toggle(item)}>{item.is_active ? <CheckCircle size={14} /> : <AlertCircle size={14} />}</AdminIconButton>}{canDelete && <AdminIconButton ariaLabel="Excluir tipo" title="Excluir" variant="danger" onClick={() => setDelId(item.id)}><Trash2 size={14} /></AdminIconButton>}</div></td>}</tr>; })}</tbody></table></div><PaginationBar page={safePage} pageSize={pageSize} totalItems={items.length} onPageChange={setPage} onPageSizeChange={size => { setPageSize(size); setPage(1); }} /></>}</AdminCard>}
