...
function ServiceTypesAdminPanelContent() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await saveServiceType({ serviceTypeId: editItem?.id, payload: { title: form.title.trim(), description: form.description.trim() || null, forecast_days: form.forecast_days ? Number(form.forecast_days) : null, is_active: form.is_active }, sortOrder: items.length, selectedSituations });
      setFormOpen(false); setToast({ msg: editItem ? "Tipo atualizado." : "Tipo criado.", type: "success" }); await refresh();
  ...
  return <div className="space-y-5">
    ...
    {delId && <ConfirmDialog message="Excluir este tipo de atendimento? OS relacionadas ficarão sem tipo." onConfirm={() => { setDelId(null); void remove(delId); }} onCancel={() => setDelId(null)} />}
    <PageHeader title="Tipos de Atendimento" subtitle="Configuração dos tipos utilizados nas ordens de serviço" actions={canCreate ? <AdminButton onClick={openNew} className="text-xs"><Plus size={15} /> Novo tipo</AdminButton> : null} />
    {canViewTable && <AdminCard>{loading ? <LoadingState /> : items.length === 0 ? <EmptyState icon={List} title="Nenhum tipo cadastrado" message="Crie tipos para disponibilizá-los na Nova OS." onAdd={canCreate ? openNew : undefined} addLabel="Novo tipo" /> : <><div className="overflow-x-auto"><table className="min-w-[820px]"><thead><tr><th className="text-left">Tipo de atendimento</th><th className="text-left">Descrição</th><th className="text-left">Previsão</th><th className="text-left">SLA</th><th className="text-left">Status</th><th className="text-right">Ações</th></tr></thead><tbody>{pagedItems.map(item => { const summary = slaSummary(item); return <tr key={item.id}><td><p className="font-bold text-[#0d1b2e]">{item.title}</p></td><td><p className="max-w-sm break-words text-xs leading-relaxed text-[#5a6a82]">{item.description || "Sem descrição"}</p></td><td className="text-xs text-[#5a6a82]">{item.forecast_days == null ? "Não informada" : `${item.forecast_days} dia(s)`}</td><td className="text-xs text-[#5a6a82]">{summary.count ? `${summary.count} situações · ${summary.totalHours}h` : "Não configurado"}</td><td><StatusBadge status={item.is_active ? "Ativo" : "Inativo"} /></td><td><div className="flex items-center justify-end gap-1">{canViewDetails && canEdit && <AdminIconButton ariaLabel="Editar tipo" title="Editar" onClick={() => openEdit(item)}><Edit2 size={14} /></AdminIconButton>}{canToggleActive && <AdminIconButton ariaLabel={item.is_active ? "Desativar tipo" : "Ativar tipo"} title={item.is_active ? "Desativar" : "Ativar"} onClick={() => toggle(item)}>{item.is_active ? <CheckCircle size={14} /> : <AlertCircle size={14} />}</AdminIconButton>}{canDelete && <AdminIconButton ariaLabel="Excluir tipo" title="Excluir" variant="danger" onClick={() => setDelId(item.id)}><Trash2 size={14} /></AdminIconButton>}</div></td></tr>; })}</tbody></table></div><PaginationBar page={safePage} pageSize={pageSize} totalItems={items.length} onPageChange={setPage} onPageSizeChange={size => { setPageSize(size); setPage(1); }} /></>}</AdminCard>}
    <AdminPage open={formOpen} onClose={() => setFormOpen(false)} breadcrumb="Operação > Tipos de Atendimento" title={editItem ? "Editar tipo de atendimento" : "Novo tipo de atendimento"} subtitle="Preencha os dados do tipo" maxW="max-w-xl">
      <div className="space-y-5 p-5">
        ...
        <AdminCard className={canManageSla ? "p-5 shadow-none" : "p-5 opacity-60 shadow-none"}><div className="mb-4"><p className="text-sm font-bold text-[#0d1b2e]">Situações e SLA</p><p className="mt-1 break-words text-xs leading-relaxed text-[#5a6a82]">Selecione as situações permitidas e configure o prazo máximo de cada etapa.</p>{!canManageSla && <p className="mt-1 text-xs font-semibold text-amber-700">Sem permissão para alterar o SLA.</p>}</div><div className="columns-1 gap-3 sm:columns-2">{situations.map(situation => { const selected = form.selectedSituations.find(item => item.situation_id === situation.id); const defaultHours = Number(situation.hours); const hasDefaultHours = Number.isFinite(defaultHours) && defaultHours > 0; return <AdminCard key={situation.id} className="mb-3 w-full break-inside-avoid bg-[#f8fafc] p-3 shadow-none"><label className="flex cursor-default items-center gap-2 text-sm font-semibold text-[#0d1b2e]"><Checkbox disabled={!canManageSla} checked={Boolean(selected)} onCheckedChange={checked => canManageSla && setForm(current => ({ ...current, selectedSituations: checked === true ? [...current.selectedSituations, { situation_id: situation.id, use_default_hours: hasDefaultHours, sla_hours: "" }] : current.selectedSituations.filter(item => item.situation_id !== situation.id) }))} /><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: situation.color || "#0057e7" }} /><span className="min-w-0 break-words">{situation.name}</span>{hasDefaultHours && <span className="text-xs font-normal text-[#5a6a82]">({defaultHours} horas)</span>}</label>{!hasDefaultHours && <p className="ml-6 mt-1 break-words text-xs text-amber-700">Esta situação não possui horas padrão</p>}{selected && <div className="ml-6 mt-3 space-y-2 text-xs text-[#0d1b2e]"><p className="font-bold">Horas:</p><RadioGroup disabled={!canManageSla} value={selected.use_default_hours ? "default" : "custom"} onValueChange={value => canManageSla && setForm(current => ({ ...current, selectedSituations: current.selectedSituations.map(item => item.situation_id === situation.id ? { ...item, use_default_hours: value === "default", ...(value === "default" ? { sla_hours: "" } : {}) } : item) }))} className="gap-2"><label className="flex cursor-default items-center gap-2"><RadioGroupItem value="default" disabled={!hasDefaultHours || !canManageSla} /> Manter padrão ({situation.hours == null ? "—" : `${situation.hours} horas`})</label><label className="flex cursor-default items-center gap-2"><RadioGroupItem value="custom" disabled={!canManageSla} /> Definir novo prazo</label></RadioGroup>{!selected.use_default_hours && <FHoursInput label="Prazo em horas" placeholder="00:00" disabled={!canManageSla} value={selected.sla_hours} onChange={(e: any) => canManageSla && setForm(current => ({ ...current, selectedSituations: current.selectedSituations.map(item => item.situation_id === situation.id ? { ...item, sla_hours: e.target.value } : item) }))} />}</div>}</AdminCard>; })}</div></AdminCard>
        <FToggle label="Tipo ativo" checked={form.is_active} onChange={is_active => setForm({ ...form, is_active })} />
      </div>
      <div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-5 py-4"><BtnSecondary onClick={() => setFormOpen(false)}>Cancelar</BtnSecondary>{(editItem ? canEdit : canCreate) && <BtnPrimary onClick={save} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</BtnPrimary>}</div>
    </AdminPage>
  </div>;
}
