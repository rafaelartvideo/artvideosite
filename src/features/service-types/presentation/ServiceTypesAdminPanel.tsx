...
function ServiceTypesAdminPanelContent({ routeResourceId, routeSubpage, onRouteChange }: ServiceTypesAdminPanelProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      closeForm(); setToast({ msg: editItem ? "Tipo atualizado." : "Tipo criado.", type: "success" }); await refresh();
  ...
  return <div className="space-y-5">
    ...
    <AdminPage open={formOpen} onClose={closeForm} breadcrumb="Operação > Tipos de Atendimento" title={editItem ? "Editar tipo de atendimento" : "Novo tipo de atendimento"} subtitle="Preencha os dados do tipo" maxW="max-w-xl">
      ...
      <div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-5 py-4"><BtnSecondary onClick={closeForm}>Cancelar</BtnSecondary>{(editItem ? canEdit : canCreate) && <BtnPrimary onClick={save} disabled={saving}>{saving ? "Salvando..." : "Salvar"}</BtnPrimary>}</div>
    </AdminPage>
  </div>;
}