...
function ServiceTypesAdminPanelContent() {
  ...
  const configurationQuery = useQuery({ queryKey: queryKeys.serviceTypes.configuration(), queryFn: loadServiceTypesConfiguration });
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      ...
      selectedSituations: links.map(link => ({ situation_id: link.situation_id, use_default_hours: link.use_default_hours !== false, sla_hours: link.sla_hours == null ? "" : String(link.sla_hours) })),
    });
    setFormOpen(true);
  };
  ...
  const save = async () => {
    ...
    for (const selected of selectedSituations) {
      ...
      if (selected.use_default_hours) {
        ...
        if (!Number.isFinite(hours) || hours <= 0) { setToast({ msg: "Uma das situações selecionadas não possui horas padrão.", type: "error" }); return; }
      } else {
        const hours = Number(selected.sla_hours);
        if (!Number.isFinite(hours) || hours <= 0) { setToast({ msg: "Informe um novo prazo válido para todas as situações personalizadas.", type: "error" }); return; }
      }
    }
    setSaving(true);
    const payload = { title: form.title.trim(), description: form.description.trim() || null, forecast_days: form.forecast_days ? Number(form.forecast_days) : null, is_active: form.is_active };
    try {
      await saveServiceType({ serviceTypeId: editItem?.id, payload, sortOrder: items.length, selectedSituations });
      setFormOpen(false);
  ...
  const toggle = async (item: any) => {
    ...
    try { await setServiceTypeActive(item.id, !item.is_active); await refresh(); }
    catch (error) { const message = error instanceof Error ? error.message : String(error); setToast({ msg: `Erro ao atualizar tipo: ${message}`, type: "error" }); }
  };

  const remove = async (id: string) => {
    if (!hasPermission("service_types.delete")) return;
    try { await deleteServiceType(id); await refresh(); }
  ...
  return (
    <div className="space-y-5">
      ...
      <PageHeader title="Tipos de Atendimento" subtitle="Configuração dos tipos utilizados nas ordens de serviço" actions={hasPermission("service_types.create") ? <AdminButton onClick={openNew} className="text-xs"><Plus size={15} /> Novo tipo</AdminButton> : null} />
