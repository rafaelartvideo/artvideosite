...
export function CustomersList(props: Props) {
  ...
  return <>
    ...
    <AdminSearchPanel title="Buscar clientes">
      <div className="relative min-w-0"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5a6a82]" /><input value={search} onChange={e => { onSearchChange(e.target.value); onPageChange(1); }} placeholder="Buscar por nome, CPF/CNPJ, WhatsApp ou e-mail..." className={cn(INPUT, "h-[42px] w-full py-2 pl-9 text-sm md:text-xs")} /></div>
    </AdminSearchPanel>
