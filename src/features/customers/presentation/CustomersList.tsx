...
export function CustomersList(props: Props) {
  ...
  return <>
    ...
    <AdminSearchPanel title="Buscar clientes">
      ...
      {hasFilters && <div className="mt-3 flex justify-stretch sm:justify-end"><AdminButton variant="danger" size="sm" onClick={onClearFilters} className="w-full bg-white text-red-600 hover:bg-red-50 sm:w-auto"><Eraser size={14} /> Limpar filtros</AdminButton></div>}
    </AdminSearchPanel>
      ...
      {loading ? <LoadingState /> : filtered.length === 0 ? <EmptyState icon={Users} title="Nenhum cliente encontrado" message="Ajuste os filtros para localizar o cliente desejado." onAdd={canCreate && customers.length === 0 ? onCreate : undefined} addLabel="Cadastrar Cliente" /> : <>
        <div className="divide-y divide-[#0d1b2e]/8 md:hidden">{pagedCustomers.map(customer => <article key={customer.id} role="button" tabIndex={0} onClick={() => onOpenDetail(customer)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpenDetail(customer); } }} className="min-w-0 p-4 transition-colors active:bg-[#f5f7fa]">
          <div className="flex min-w-0 items-start justify-between gap-3">
            <div className="min-w-0 flex-1">{showName && <p className="truncate text-sm font-bold text-[#0d1b2e]">{customer.full_name || "—"}</p>}{showDocument && <p className="mt-1 font-mono text-[11px] text-[#5a6a82]"><span className="font-bold text-[#0057e7]">{customer.customer_type === "PJ" ? "PJ" : "PF"}</span> · {customerDocument(customer)}</p>}</div>
            {showCreatedAt && <span className="shrink-0 text-[10px] font-medium text-[#8a96a8]">{fmtDate(customer.created_at)}</span>}
          </div>
