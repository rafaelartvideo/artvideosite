...
export function TabInventory({ routeResourceId, routeSubpage, onRouteChange }: TabInventoryProps) {
  ...
  const openEdit = async (item: any) => {
    ...
    setForm({
      ...
      conversion_factor: String(conversionFactor(item)),
  ...
  const saveMovement = async () => {
    ...
    try {
      ...
      const inputUnit = movementType === "ADJUST" ? (currentItem.unit === "cx" ? "cx" : "un") : movementForm.input_unit;
      await recordInventoryMovement({
        inventory_item_id: selectedItem.id,
        movement_type: movementType,
        input_quantity: quantity,
        input_unit: inputUnit,
        supplier_entity_id: movementType === "IN" ? (movementForm.supplier_entity_id || null) : null,
