...
import { AlertCircle, ArrowLeftRight, Check, CheckCircle, ChevronDown, Edit2, Eraser, List, MapPin, Package, Plus, Search } from "lucide-react";
import { useAuth } from "@/lib/auth";
import { queryKeys } from "@/infrastructure/query/query-keys";
import { getInventoryItem, listInventoryItems, listInventoryMovements, recordInventoryMovement, saveInventoryItem, setInventoryItemActive } from "../infrastructure/inventory.repository";
...
export function TabInventory({ routeResourceId, routeSubpage, onRouteChange }: TabInventoryProps) {
  ...
  const saveItem = async () => {
    ...
    if (!selectedItem && (!Number.isInteger(initialQuantity) || initialQuantity < 0)) { setToast({ msg: "Informe uma quantidade inicial inteira e não negativa.", type: "error" }); return; }
  ...
  const saveMovement = async () => {
    ...
    if (!Number.isInteger(quantity) || quantity <= 0) { setToast({ msg: "Informe uma quantidade inteira maior que zero para a movimentação.", type: "error" }); return; }
    let currentItem;
    ...
    if (movementType === "OUT") { if (current < quantity) { setToast({ msg: `Estoque insuficiente para saída: há ${formatNumber(current)} ${unitLabel(currentItem.unit)} disponível(is).`, type: "error" }); return; } nextQuantity = current - quantity; }
    if (movementType === "ADJUST") nextQuantity = quantity;
    try {
      await recordInventoryMovement({ inventory_item_id: selectedItem.id, movement_type: movementType, input_quantity: quantity, reason: movementForm.reason.trim() || "Movimentação manual", service_order_id: movementForm.service_order_id || null });
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {recordOpen && <AdminPage open onClose={closePage} breadcrumb="Estoque" title={selectedItem ? "Editar item" : "Novo item"} subtitle={selectedItem ? "Atualize os dados cadastrais e o endereço físico da peça." : "Cadastre a peça, unidade, saldo inicial e endereço físico."} maxW="max-w-3xl"><div className="space-y-5 p-4 sm:p-5">
      ...
      {!selectedItem && <Section title="Controle de estoque"><div className="space-y-4"><div><label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wider text-[#5a6a82]">Unidade</label><AdminSelect value={form.unit} onValueChange={unit => setForm(current => ({ ...current, unit: unit === "cx" ? "cx" : "un", conversion_factor: unit === "cx" ? (current.conversion_factor === "1" ? "" : current.conversion_factor) : "1" }))} options={[{ value: "un", label: "Unidade (un)" }, { value: "cx", label: "Caixa (cx)" }]} ariaLabel="Unidade do item" /></div>{form.unit === "cx" && <FIntegerInput label="Unidades por caixa" required value={form.conversion_factor} onChange={(e: any) => setForm({ ...form, conversion_factor: e.target.value })} />}<div className="grid gap-4 sm:grid-cols-2"><FIntegerInput label={`Quantidade inicial (${form.unit})`} value={form.quantity} onChange={(e: any) => setForm({ ...form, quantity: e.target.value })} /><FIntegerInput label={`Quantidade mínima (${form.unit})`} value={form.min_quantity} onChange={(e: any) => setForm({ ...form, min_quantity: e.target.value })} /></div>{form.unit === "cx" && Number(form.conversion_factor) > 0 && <AdminCard className="bg-[#f8fafc] p-3 shadow-none"><p className="text-xs font-semibold text-[#5a6a82]">{formatNumber(Number(form.quantity || 0))} cx = {formatNumber(Number(form.quantity || 0) * Number(form.conversion_factor || 0))} un</p></AdminCard>}</div></Section>}
      {selectedItem && <Section title="Controle de estoque"><FIntegerInput label={`Quantidade mínima (${form.unit})`} value={form.min_quantity} onChange={(e: any) => setForm({ ...form, min_quantity: e.target.value })} /></Section>}
    ...
    {selectedItem && !recordOpen && !historyOpen && canCreateMovements && routeSubpage === "move" && <AdminPage open onClose={closePage} breadcrumb="Estoque" title={`Movimentação — ${selectedItem.name}`} subtitle="Registre entrada, saída ou ajuste do saldo" maxW="max-w-xl"><div className="space-y-4 p-4 sm:p-5"><div className="grid gap-3 sm:grid-cols-2"><AdminCard className="bg-[#f8fafc] p-4 shadow-none"><p className="text-[10px] font-bold uppercase text-[#5a6a82]">Quantidade atual</p><p className="mt-1 text-lg font-black">{formatNumber(Number(selectedItem.quantity ?? 0))} {unitLabel(selectedItem.unit)}</p></AdminCard><AdminCard className="bg-[#f8fafc] p-4 shadow-none"><p className="text-[10px] font-bold uppercase text-[#5a6a82]">Endereço</p><p className="mt-1 text-sm font-bold">{storageAddress(selectedItem) || "Não informado"}</p></AdminCard></div><div><label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wider text-[#5a6a82]">Tipo de movimentação</label><AdminSelect value={movementForm.type} onValueChange={type => setMovementForm({ ...movementForm, type })} options={[{ value: "in", label: "Entrada" }, { value: "out", label: "Saída" }, { value: "adjust", label: "Ajuste" }]} ariaLabel="Tipo de movimentação" /></div><FIntegerInput label={`Quantidade (${unitLabel(selectedItem.unit)})`} value={movementForm.quantity} onChange={(e: any) => setMovementForm({ ...movementForm, quantity: e.target.value })} />{selectedItem.unit === "cx" && movementQuantity > 0 && <AdminCard className="border-blue-100 bg-blue-50 p-3 shadow-none"><p className="text-sm font-black text-blue-800">{formatNumber(movementQuantity)} cx × {formatNumber(conversionFactor(selectedItem))} = {formatNumber(equivalentUnits(movementQuantity, selectedItem))} un</p></AdminCard>}<FInput label="Motivo" value={movementForm.reason} onChange={(e: any) => setMovementForm({ ...movementForm, reason: e.target.value })} /><FInput label="OS relacionada (opcional)" value={movementForm.service_order_id} onChange={(e: any) => setMovementForm({ ...movementForm, service_order_id: e.target.value })} /></div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closePage}>Cancelar</BtnSecondary><BtnPrimary onClick={saveMovement}>Registrar</BtnPrimary></div></AdminPage>}

    {historyOpen && selectedItem && canViewMovements && <AdminPage open onClose={closePage} breadcrumb="Estoque" title={`Histórico — ${selectedItem.name}`} subtitle="Movimentações do item" maxW="max-w-2xl"><div className="p-4 sm:p-5">{history.length === 0 ? <p className="text-sm text-[#5a6a82]">Nenhuma movimentação registrada.</p> : <div className="space-y-3">{history.map((entry: any) => { const presentation = movementPresentation(entry.movement_type); const qty = Number(entry.quantity || 0); const displayUnit = entry.display_unit || unitLabel(selectedItem.unit); return <AdminCard key={entry.id} className="bg-[#f8fafc] p-4 shadow-none"><div className="flex items-center justify-between gap-3"><span className={cn("rounded-full px-2.5 py-1 text-[10px] font-black uppercase", presentation.badge)}>{presentation.label}</span><span className={cn("text-sm font-black", presentation.text)}>{presentation.sign}{formatNumber(qty)} {displayUnit}</span></div>{entry.base_quantity != null && displayUnit === "cx" && <div className="mt-2 text-[11px] font-semibold text-[#5a6a82]">Equivalente: {formatNumber(Number(entry.base_quantity))} un</div>}<div className="mt-2 text-sm text-[#0d1b2e]">{entry.reason || "Movimentação manual"}</div><div className="mt-2 grid gap-2 text-[11px] text-[#5a6a82] sm:grid-cols-2"><div><strong>Data:</strong> {formatDateTime(entry.created_at)}</div><div><strong>Usuário:</strong> {entry.created_by_profile?.full_name || "—"}</div><div><strong>OS:</strong> {entry.service_order?.os_number || "—"}</div><div><strong>Endereço:</strong> {storageAddress(selectedItem) || "—"}</div></div></AdminCard>; })}</div>}</div></AdminPage>}
  </div>;
}
