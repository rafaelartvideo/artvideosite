...
import { AlertCircle, ArrowLeftRight, CheckCircle, Edit2, List, Package, Plus, Trash2 } from "lucide-react";
...
export function TabInventory({ onBack, routeResourceId, routeSubpage, onRouteChange }: TabInventoryProps) {
  ...
  const saveMovement = async () => {
    ...
    if (movementType === "OUT") {
      if (current < quantity) { setToast({ msg: `Estoque insuficiente para saída: há ${current} ${unitLabel(currentItem.unit)} disponível(is).`, type: "error" }); return; }
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {!routeResourceId && <><PageHeader title="Estoque" subtitle="Controle de itens, quantidade mínima e movimentações do almoxarifado" actions={<div className="flex items-center gap-2">{onBack && <InternalBackButton onBack={onBack} inHeader />}{canCreate && <AdminButton onClick={openNewPage} className="text-xs"><Plus size={14} /> Novo item</AdminButton>}</div>} />
    {canViewTable && <AdminCard>{itemsQuery.isPending ? <LoadingState /> : items.length === 0 ? <EmptyState icon={Package} title="Nenhum item em estoque" message="Cadastre um item para começar a controlar o inventário." /> : <>
      <div className="divide-y divide-[#0d1b2e]/8 md:hidden">{pagedItems.map((item: any) => { const quantity = Number(item.quantity ?? 0); const minQuantity = Number(item.min_quantity ?? 0); const lowStock = quantity <= minQuantity; const isEmpty = quantity === 0; const stockLabel = isEmpty ? "Sem estoque" : lowStock ? "Estoque baixo" : "Estoque normal"; const factor = conversionFactor(item); return <article key={item.id} className="min-w-0 space-y-3 overflow-hidden p-4">
        <div className="flex min-w-0 items-start justify-between gap-3">
          <div className="flex min-w-0 flex-1 items-start gap-2.5">
            <span className={cn("mt-0.5 h-10 w-1.5 shrink-0 rounded-full", isEmpty ? "bg-red-500" : lowStock ? "bg-amber-500" : "bg-emerald-500")} />
            <div className="min-w-0 flex-1">
              {showName && <p className="break-words text-sm font-black leading-tight text-[#0d1b2e]">{item.name}</p>}
              <div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">{showSku && <span className="font-mono text-[10px] font-semibold text-[#5a6a82]">SKU {item.sku || "—"}</span>}{item.description && <span className="min-w-0 truncate text-[10px] text-[#8a96a8]">{item.description}</span>}</div>
            </div>
          </div>
          {showStatus && <span className={cn("shrink-0 rounded-full px-2 py-1 text-[10px] font-bold", item.is_active !== false ? "bg-green-100 text-green-700" : "bg-[#f5f7fa] text-[#5a6a82]")}>{item.is_active !== false ? "Ativo" : "Inativo"}</span>}
        </div>

        {showQuantity && <div className={cn("flex min-w-0 items-center justify-between gap-3 rounded-xl border px-3 py-2.5", isEmpty ? "border-red-200 bg-red-50" : lowStock ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50")}><div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wider text-[#5a6a82]">Quantidade atual</p><p className={cn("mt-0.5 text-xl font-black", isEmpty ? "text-red-700" : lowStock ? "text-amber-700" : "text-emerald-700")}>{quantity}<span className="ml-1 text-[11px] font-bold">{showUnit ? unitLabel(item.unit) : ""}</span></p>{item.unit === "cx" && <p className="mt-0.5 text-[10px] font-semibold text-[#5a6a82]">{equivalentUnits(quantity, item)} un no total</p>}</div><span className={cn("shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase", isEmpty ? "bg-red-100 text-red-700" : lowStock ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700")}>{stockLabel}</span></div>}

        <div className="grid min-w-0 grid-cols-2 gap-x-4 gap-y-2 text-xs">
          {showMinQuantity && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Estoque mínimo</p><p className="truncate font-semibold text-[#34445b]">{minQuantity}{showUnit ? ` ${unitLabel(item.unit)}` : ""}</p></div>}
          {showUnit && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Unidade</p><p className="truncate font-semibold text-[#34445b]">{unitLabel(item.unit)}{item.unit === "cx" ? ` · ${factor} un/cx` : ""}</p></div>}
          {showPurchasePrice && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Compra</p><p className="truncate font-semibold text-[#34445b]">{formatCurrency(item.purchase_price)}</p></div>}
          {showSalePrice && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Venda</p><p className="truncate font-semibold text-[#34445b]">{formatCurrency(item.sale_price)}</p></div>}
        </div>

        {showActions && (canViewDetails && canEdit || canCreateMovements || canViewMovements || canToggleActive || canDelete) && <div className="border-t border-[#0d1b2e]/8 pt-3" onClick={event => event.stopPropagation()}>
          {(canViewDetails && canEdit || canCreateMovements) && <div className="grid grid-cols-2 gap-2">
            {canViewDetails && canEdit ? <AdminButton variant="secondary" size="sm" onClick={() => openEditPage(item)} className="h-10 w-full justify-center border-[#0057e7]/25 text-xs font-bold text-[#0057e7] hover:bg-[#0057e7]/5"><Edit2 size={15} /> Editar</AdminButton> : <span />}
            {canCreateMovements ? <AdminButton size="sm" onClick={() => openMovementPage(item)} className="h-10 w-full justify-center text-xs font-bold"><ArrowLeftRight size={15} /> Movimentar</AdminButton> : <span />}
          </div>}
          {(canViewMovements || canToggleActive || canDelete) && <div className="mt-2 flex min-w-0 items-center justify-end gap-1.5">
            {canViewMovements && <AdminIconButton ariaLabel="Ver movimentações" title="Histórico" onClick={() => openHistoryPage(item)} className="h-10 w-10"><List size={16} /></AdminIconButton>}
            {canToggleActive && <AdminIconButton ariaLabel={item.is_active !== false ? "Desativar item" : "Ativar item"} title={item.is_active !== false ? "Desativar" : "Ativar"} onClick={() => toggleActive(item)} className="h-10 w-10">{item.is_active !== false ? <CheckCircle size={16} /> : <AlertCircle size={16} />}</AdminIconButton>}
            {canDelete && <AdminIconButton ariaLabel="Excluir item" title="Excluir" variant="danger" onClick={() => void deleteItem(item)} className="h-10 w-10"><Trash2 size={16} /></AdminIconButton>}
          </div>}
        </div>}
      </article>; })}</div>

      <div className="hidden overflow-x-auto md:block"><table className="min-w-[980px]"><thead><tr>{showName && <th className="text-left">Nome</th>}{showSku && <th className="text-left">SKU</th>}{showUnit && <th className="text-left">Unidade</th>}{showQuantity && <th className="text-left">Quantidade</th>}{showMinQuantity && <th className="text-left">Mínimo</th>}{showPurchasePrice && <th className="text-left">Compra</th>}{showSalePrice && <th className="text-left">Venda</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedItems.map((item: any) => { const quantity = Number(item.quantity ?? 0); const minQuantity = Number(item.min_quantity ?? 0); const lowStock = quantity <= minQuantity; const isEmpty = quantity === 0; const factor = conversionFactor(item); return <tr key={item.id}>{showName && <td><div className="font-semibold text-[#0d1b2e]">{item.name}</div>{item.description && <div className="max-w-sm break-words text-[11px] leading-relaxed text-[#5a6a82]">{item.description}</div>}</td>}{showSku && <td className="font-mono text-xs text-[#5a6a82]">{item.sku || "—"}</td>}{showUnit && <td className="text-xs text-[#5a6a82]">{unitLabel(item.unit)}{item.unit === "cx" && <div className="mt-0.5 text-[10px]">{factor} un/cx</div>}</td>}{showQuantity && <td><span className={cn("text-sm font-bold", isEmpty ? "text-red-700" : lowStock ? "text-amber-700" : "text-[#0d1b2e]")}>{quantity}</span>{isEmpty && <span className="ml-2 text-[10px] font-bold uppercase text-red-700">Sem estoque</span>}{!isEmpty && lowStock && <span className="ml-2 text-[10px] font-bold uppercase text-amber-700">Baixo</span>}{item.unit === "cx" && <div className="mt-0.5 text-[10px] text-[#5a6a82]">{equivalentUnits(quantity, item)} un</div>}</td>}{showMinQuantity && <td className="text-xs text-[#5a6a82]">{minQuantity}</td>}{showPurchasePrice && <td className="text-xs text-[#5a6a82]">{formatCurrency(item.purchase_price)}</td>}{showSalePrice && <td className="text-xs text-[#5a6a82]">{formatCurrency(item.sale_price)}</td>}{showStatus && <td><span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold", item.is_active !== false ? "bg-green-100 text-green-700" : "bg-[#f5f7fa] text-[#5a6a82]")}>{item.is_active !== false ? "Ativo" : "Inativo"}</span></td>}{showActions && <td><div className="flex justify-end gap-1">{canViewDetails && canEdit && <AdminIconButton ariaLabel="Editar item" title="Editar" onClick={() => openEditPage(item)}><Edit2 size={14} /></AdminIconButton>}{canToggleActive && <AdminIconButton ariaLabel={item.is_active !== false ? "Desativar item" : "Ativar item"} title={item.is_active !== false ? "Desativar" : "Ativar"} onClick={() => toggleActive(item)}>{item.is_active !== false ? <CheckCircle size={14} /> : <AlertCircle size={14} />}</AdminIconButton>}{canDelete && <AdminIconButton ariaLabel="Excluir item" title="Excluir" variant="danger" onClick={() => void deleteItem(item)}><Trash2 size={14} /></AdminIconButton>}{canViewMovements && <AdminIconButton ariaLabel="Movimentações" title="Movimentações" onClick={() => openHistoryPage(item)}><List size={14} /></AdminIconButton>}{canCreateMovements && <AdminIconButton ariaLabel="Movimentar item" title="Movimentar item" onClick={() => openMovementPage(item)}><ArrowLeftRight size={14} /></AdminIconButton>}</div></td>}</tr>; })}</tbody></table></div>
    ...
    {recordOpen && <AdminPage open={true} onClose={closePage} breadcrumb="Estoque" title={selectedItem ? "Editar item" : "Novo item"} subtitle="Cadastro do item em estoque" maxW="max-w-xl"><div className="space-y-4 p-4 sm:p-5">
      ...
      <div><label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wider text-[#5a6a82]">Unidade</label><AdminSelect value={form.unit} onValueChange={unit => setForm(current => ({ ...current, unit: unit === "cx" ? "cx" : "un", conversion_factor: unit === "cx" ? (current.conversion_factor === "1" ? "" : current.conversion_factor) : "1" }))} options={[{ value: "un", label: "Unidade (un)" }, { value: "cx", label: "Caixa (cx)" }]} ariaLabel="Unidade do item" /></div>
      {form.unit === "cx" && <div><FInput label="Unidades por caixa" required type="number" min="1" step="1" value={form.conversion_factor} onChange={(e: any) => setForm({ ...form, conversion_factor: e.target.value })} /><p className="mt-1.5 text-xs leading-relaxed text-[#718096]">Informe a quantidade exata de unidades contidas em cada caixa. Ex.: 1 cx × 12 = 12 un.</p></div>}
      <div className="grid gap-4 sm:grid-cols-2"><FInput label={`Quantidade (${form.unit})`} type="number" min="0" value={form.quantity} onChange={(e: any) => setForm({ ...form, quantity: e.target.value })} /><FInput label={`Quantidade mínima (${form.unit})`} type="number" min="0" value={form.min_quantity} onChange={(e: any) => setForm({ ...form, min_quantity: e.target.value })} /></div>
      {form.unit === "cx" && Number(form.conversion_factor) > 0 && <AdminCard className="bg-[#f8fafc] p-3 shadow-none"><p className="text-[10px] font-bold uppercase text-[#5a6a82]">Equivalência do saldo informado</p><p className="mt-1 text-sm font-black text-[#0d1b2e]">{Number(form.quantity || 0)} cx = {Number(form.quantity || 0) * Number(form.conversion_factor || 0)} un</p></AdminCard>}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FCurrencyInput label="Valor de compra" value={form.purchase_price} onChange={(e: any) => setForm({ ...form, purchase_price: e.target.value })} /><FCurrencyInput label="Valor de venda" value={form.sale_price} onChange={(e: any) => setForm({ ...form, sale_price: e.target.value })} /></div>
    ...
    </div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closePage}>Cancelar</BtnSecondary>{(selectedItem ? canEdit : canCreate) && <BtnPrimary onClick={saveItem}>{selectedItem ? "Salvar" : "Cadastrar"}</BtnPrimary>}</div></AdminPage>}

    {selectedItem && !recordOpen && !historyOpen && canCreateMovements && routeSubpage === "move" && <AdminPage open={true} onClose={closePage} breadcrumb="Estoque" title={`Movimentação — ${selectedItem.name}`} subtitle="Registre entrada, saída ou ajuste do saldo" maxW="max-w-xl"><div className="space-y-4 p-4 sm:p-5">
      <div className="grid gap-3 text-sm sm:grid-cols-2"><AdminCard className="bg-[#f8fafc] p-4 shadow-none"><p className="text-[10px] font-bold uppercase text-[#5a6a82]">Quantidade atual</p><p className="mt-1 text-lg font-black text-[#0d1b2e]">{Number(selectedItem.quantity ?? 0)} {unitLabel(selectedItem.unit)}</p>{selectedItem.unit === "cx" && <p className="mt-1 text-[11px] font-semibold text-[#5a6a82]">{equivalentUnits(Number(selectedItem.quantity ?? 0), selectedItem)} un</p>}</AdminCard><AdminCard className="bg-[#f8fafc] p-4 shadow-none"><p className="text-[10px] font-bold uppercase text-[#5a6a82]">Mínimo</p><p className="mt-1 text-lg font-black text-[#0d1b2e]">{Number(selectedItem.min_quantity ?? 0)} {unitLabel(selectedItem.unit)}</p>{selectedItem.unit === "cx" && <p className="mt-1 text-[11px] font-semibold text-[#5a6a82]">1 cx = {conversionFactor(selectedItem)} un</p>}</AdminCard></div>
      <div><label className="mb-1.5 block text-[11px] font-bold uppercase tracking-wider text-[#5a6a82]">Tipo de movimentação</label><AdminSelect value={movementForm.type} onValueChange={type => setMovementForm({ ...movementForm, type })} options={[{ value: "in", label: "Entrada" }, { value: "out", label: "Saída" }, { value: "adjust", label: "Ajuste" }]} className="text-xs" ariaLabel="Tipo de movimentação" /></div>
      <FInput label={`Quantidade (${unitLabel(selectedItem.unit)})`} type="number" min="1" value={movementForm.quantity} onChange={(e: any) => setMovementForm({ ...movementForm, quantity: e.target.value })} />
      {selectedItem.unit === "cx" && movementQuantity > 0 && <AdminCard className="border-blue-100 bg-blue-50 p-3 shadow-none"><p className="text-[10px] font-bold uppercase text-blue-700">Fator de conversão</p><p className="mt-1 text-sm font-black text-blue-800">{movementQuantity} cx × {conversionFactor(selectedItem)} = {equivalentUnits(movementQuantity, selectedItem)} un</p></AdminCard>}
    ...
    </div><div className="sticky bottom-0 flex justify-end gap-3 border-t border-[#0d1b2e]/8 bg-white px-4 py-4 sm:px-5"><BtnSecondary onClick={closePage}>Cancelar</BtnSecondary><BtnPrimary onClick={saveMovement}>Registrar</BtnPrimary></div></AdminPage>}

    {historyOpen && selectedItem && canViewMovements && <AdminPage open={true} onClose={closePage} breadcrumb="Estoque" title={`Histórico — ${selectedItem.name}`} subtitle="Movimentações do item" maxW="max-w-2xl"><div className="p-4 sm:p-5">{history.length === 0 ? <p className="text-sm text-[#5a6a82]">Nenhuma movimentação registrada para este item.</p> : <div className="space-y-3">{history.map((entry: any) => { const presentation = movementPresentation(entry.movement_type); const qty = Number(entry.quantity || 0); return <AdminCard key={entry.id} className="bg-[#f8fafc] p-4 shadow-none"><div className="flex items-center justify-between gap-3"><span className={cn("rounded-full px-2.5 py-1 text-[10px] font-black uppercase", presentation.badge)}>{presentation.label}</span><span className={cn("text-sm font-black", presentation.text)}>{presentation.sign}{qty} {unitLabel(selectedItem.unit)}</span></div>{selectedItem.unit === "cx" && <div className="mt-2 text-[11px] font-semibold text-[#5a6a82]">Equivalente: {equivalentUnits(qty, selectedItem)} un ({conversionFactor(selectedItem)} un/cx)</div>}<div className="mt-2 break-words text-sm text-[#0d1b2e]">{entry.reason || "Movimentação manual"}</div><div className="mt-2 grid gap-2 text-[11px] text-[#5a6a82] sm:grid-cols-2"><div><span className="font-bold">Data:</span> {entry.created_at ? new Date(entry.created_at).toLocaleString("pt-BR") : "—"}</div><div><span className="font-bold">Usuário:</span> {entry.created_by_profile?.full_name || "—"}</div><div><span className="font-bold">OS:</span> {entry.service_order?.os_number || "—"}</div><div><span className="font-bold">Quantidade:</span> {qty} {unitLabel(selectedItem.unit)}</div></div></AdminCard>; })}</div>}</div></AdminPage>}
  </div>;
}
