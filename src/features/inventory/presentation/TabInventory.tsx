...
export function TabInventory({ onBack, routeResourceId, routeSubpage, onRouteChange }: TabInventoryProps) {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {canViewTable && <AdminCard>{itemsQuery.isPending ? <LoadingState /> : items.length === 0 ? <EmptyState icon={Package} title="Nenhum item em estoque" message="Cadastre um item para começar a controlar o inventário." /> : <>
      <div className="divide-y divide-[#0d1b2e]/8 md:hidden">{pagedItems.map((item: any) => { const quantity = Number(item.quantity ?? 0); const minQuantity = Number(item.min_quantity ?? 0); const lowStock = quantity <= minQuantity; const isEmpty = quantity === 0; const stockLabel = isEmpty ? "Sem estoque" : lowStock ? "Estoque baixo" : "Estoque normal"; return <article key={item.id} className="min-w-0 space-y-3 overflow-hidden p-4">
        ...
        {showQuantity && <div className={cn("flex min-w-0 items-center justify-between gap-3 rounded-xl border px-3 py-2.5", isEmpty ? "border-red-200 bg-red-50" : lowStock ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50")}><div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wider text-[#5a6a82]">Quantidade atual</p><p className={cn("mt-0.5 text-xl font-black", isEmpty ? "text-red-700" : lowStock ? "text-amber-700" : "text-emerald-700")}>{quantity}<span className="ml-1 text-[11px] font-bold">{showUnit ? item.unit || "un" : ""}</span></p></div><span className={cn("shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase", isEmpty ? "bg-red-100 text-red-700" : lowStock ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700")}>{stockLabel}</span></div>}

        <div className="grid min-w-0 grid-cols-2 gap-x-4 gap-y-2 text-xs">
          {showMinQuantity && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Estoque mínimo</p><p className="truncate font-semibold text-[#34445b]">{minQuantity}{showUnit ? ` ${item.unit || "un"}` : ""}</p></div>}
          {showUnit && <div className="min-w-0"><p className="text-[9px] font-bold uppercase tracking-wide text-[#8a96a8]">Unidade</p><p className="truncate font-semibold text-[#34445b]">{item.unit || "un"}</p></div>}
