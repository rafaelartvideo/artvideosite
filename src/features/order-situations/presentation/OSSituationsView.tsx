...
export function OSSituationsView({ onBack }: { onBack: () => void }) {
  ...
  const showSlug = hasPermission("situations.table.slug");
  ...
  const situationsQuery = useQuery({ queryKey: queryKeys.orderSituations.lists(), queryFn: listOrderSituations, enabled: canView && canViewTable });
  ...
  const save = async () => {
    ...
    try { if (editItem) await updateOrderSituation(editItem.id, payload); else await createOrderSituation(payload); setDrawerOpen(false); setToast({ msg: editItem ? "Situação atualizada." : "Situação criada.", type: "success" }); await refresh(); }
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {canViewTable && <AdminCard>{loading ? <LoadingState /> : items.length === 0 ? <EmptyState icon={List} title="Nenhuma situação" message="Crie situações para acompanhar as etapas das OS." /> : <><div className="overflow-x-auto"><table className="min-w-[720px]"><thead><tr>{showSortOrder && <th className="text-left">Ordem</th>}{showSituation && <th className="text-left">Situação</th>}{showSlug && <th className="text-left">Slug</th>}{showSla && <th className="text-left">Prazo</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedItems.map(item => <tr key={item.id}>{showSortOrder && <td className="font-mono text-xs text-[#5a6a82]">{item.sort_order}</td>}{showSituation && <td><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: item.color || "#0057e7" }} /><span className="font-semibold text-[#0d1b2e]">{item.name}</span></div></td>}{showSlug && <td className="font-mono text-xs text-[#5a6a82]">{item.slug || "—"}</td>}{showSla && <td className="text-xs text-[#5a6a82]">{item.hours == null ? "Não informado" : `${item.hours} hora(s)`}</td>}{showStatus && <td><span className={cn("rounded-full px-2 py-0.5 text-[10px] font-bold", item.is_active ? "bg-green-100 text-green-700" : "bg-[#f5f7fa] text-[#5a6a82]")}>{item.is_active ? "Ativa" : "Inativa"}</span></td>}{showActions && <td><div className="flex justify-end gap-1">{canEdit && <AdminIconButton ariaLabel="Editar situação" title="Editar" onClick={() => openEdit(item)}><Edit2 size={14} /></AdminIconButton>}{canDelete && <AdminIconButton ariaLabel="Excluir situação" title="Excluir" variant="danger" onClick={() => setDelId(item.id)}><Trash2 size={14} /></AdminIconButton>}</div></td>}</tr>)}</tbody></table></div><PaginationBar page={safePage} pageSize={pageSize} totalItems={items.length} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1); }} /></>}</AdminCard>}
