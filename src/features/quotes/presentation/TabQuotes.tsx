...
export function TabQuotes({ onNavigate, routeResourceId, onRouteChange }: TabQuotesProps) {
  ...
  return <div className="min-w-0 space-y-5">
    ...
    {!routeResourceId && <>
      ...
      {canViewTable && <AdminSearchPanel title="Buscar orçamentos"><div className="grid min-w-0 gap-3 md:grid-cols-[minmax(0,1fr)_220px]"><div className="relative min-w-0"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5a6a82]" /><input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Cliente, CPF/CNPJ, WhatsApp ou protocolo" className={cn(INPUT, "h-[42px] w-full py-2 pl-9 text-sm md:text-xs")} /></div><AdminSelect value={filterStatus} onValueChange={value => { setFilterStatus(value); setPage(1); }} options={[{ value: "", label: "Todos os status" }, ...statuses.map(status => ({ value: status.id, label: status.name }))]} className="h-[42px] text-xs" ariaLabel="Filtrar orçamentos por status" /></div></AdminSearchPanel>}
