...
export function TabPartnerCompanies({ onBack, routeResourceId, onRouteChange }: RouteProps) {
  ...
  return <div className="space-y-5"><AreaHeader title={item.label} description={item.description} onBack={closeArea} />{area === "companies" && <CompaniesArea />}{area === "users" && <PartnerUsersArea />}{area === "permissions" && <PartnerPermissionsArea />}</div>;
}
