...
export function PartnerCompanyPermissionsSection({ organizationId }: { organizationId:string }) {
  ...
  return <div className="space-y-4">
    ...
    <AdminCard>
      <AdminCardHeader><div><h3 className="text-sm font-black text-[#0d1b2e]">Módulos disponíveis</h3><p className="mt-0.5 text-xs text-[#5a6a82]">Define quais áreas esta empresa pode utilizar.</p></div></AdminCardHeader>
    ...
    <AdminCard>
      <AdminCardHeader><div><h3 className="text-sm font-black text-[#0d1b2e]">Dados compartilhados com a ArtVideo</h3><p className="mt-0.5 text-xs text-[#5a6a82]">Configuração independente dos módulos liberados para a empresa.</p></div></AdminCardHeader>
      <AdminCardContent>{sharesQuery.isPending?<LoadingState/>:sharesQuery.isError?<p className="text-sm font-semibold text-red-700">{(sharesQuery.error as any)?.message||"Não foi possível carregar os compartilhamentos."}</p>:<div className="grid divide-y divide-[#d9e1ec] md:grid-cols-3 md:divide-x md:divide-y-0">{SHARE_RESOURCES.map((resource,index)=>{const level=shareByKey.get(resource.key)||"none";const spacing=index===0?"pb-4 md:pb-0 md:pr-5":index===SHARE_RESOURCES.length-1?"pt-4 md:pt-0 md:pl-5":"py-4 md:px-5 md:py-0";return <div key={resource.key} className={spacing}><p className="text-sm font-black text-[#0d1b2e]">{resource.label}</p><p className="mt-1 min-h-0 text-xs leading-relaxed text-[#5a6a82] md:min-h-[2.5rem]">{resource.description}</p><div className="mt-4"><FSelect label="Acesso da ArtVideo" value={level} options={ACCESS_OPTIONS} disabled={!canManageShares||busy} onChange={(e:any)=>shareMutation.mutate({resourceKey:resource.key,accessLevel:e.target.value as PartnerShareAccessLevel})}/></div><p className="mt-3 text-xs leading-relaxed text-[#5a6a82]">{ACCESS_HELP[level]}</p></div>;})}</div>}</AdminCardContent>
    </AdminCard>
  </div>;
}
