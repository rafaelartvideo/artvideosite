...
import { ArrowLeftRight, Edit2, Eraser, History, MapPin, Package, Plus } from "lucide-react";
...
const emptyForm = (): InventoryForm => ({
  name: "", sku: "", description: "", unit: "un", conversion_factor: "1", quantity: "0", min_quantity: "0",
...
export function PartnerInventoryData({ organizationId, accessMode }: { organizationId: string; accessMode: AccessMode }) {
  ...
  const showActions = hasPermission("inventory.table.actions") && (canViewDetails || canEdit || canMove || canViewMovements || canToggle || canDelete);
  ...
  useEffect(() => {
    setDetailItem(null); setCreating(false); setEditingItem(null); setMovementItem(null); setHistoryItem(null); setPage(1);
  }, [organizationId]);
  useEffect(() => {
  ...
  const closeItemEditor = () => { setCreating(false); setEditingItem(null); setForm(emptyForm()); };
  const openNew = () => { if (!canCreate) return; setCreating(true); setEditingItem(null); setForm(emptyForm()); };
  const openEdit = (item: any) => { if (!canEdit) return; setCreating(false); setEditingItem(item); setForm(formFromItem(item)); };
  ...
  const saveItem = async () => {
    ...
    const commonPayload = {
      ...
      unit: form.unit, conversion_factor: factor, min_quantity: minQuantity, purchase_price: purchasePrice, sale_price: salePrice,
    ...
    } catch (error) { setToast({ msg: `Erro ao salvar item: ${supabaseErrorMessage(error)}`, type: "error" }); }
  ...
  const toggleActive = async (item: any) => {
    ...
    try {
      ...
      await itemsQuery.refetch();
    } catch (error) { setToast({ msg: `Erro ao alterar status: ${supabaseErrorMessage(error)}`, type: "error" }); }
  };
  const removeItem = async (item: any) => {
    ...
    try {
      ...
      await itemsQuery.refetch();
    } catch (error) { setToast({ msg: `Erro ao excluir item: ${supabaseErrorMessage(error)}`, type: "error" }); }
  };

  const openMovement = (item: any) => { if (canMove) { setMovementItem(item); setMovementForm({ type: "in", quantity: "", reason: "" }); } };
  const saveMovement = async () => {
    ...
    try {
      ...
      await recordInventoryMovement({
        inventory_item_id: movementItem.id, movement_type: movementType, quantity,
        reason: movementForm.reason.trim() || "Movimentação manual", created_by: user?.id || null, service_order_id: null,
  ...
  const openHistory = async (item: any) => {
    ...
    try { setHistory(await listInventoryMovements(item.id, organizationId)); }
    catch (error) { setToast({ msg: `Erro ao carregar histórico: ${supabaseErrorMessage(error)}`, type: "error" }); }
  ...
  return <div className="min-w-0 space-y-4">
    ...
    <div className="flex min-w-0 items-center justify-between gap-3">
      <div className="min-w-0"><h4 className="text-base font-black text-[#0d1b2e]">Estoque</h4><p className="mt-0.5 text-xs text-[#5a6a82]">Itens e movimentações da empresa selecionada.</p></div>
    ...
    <AdminCard>
      {itemsQuery.isPending ? <LoadingState /> : filtered.length === 0 ? <EmptyState icon={Package} title={items.length ? "Nenhum item encontrado" : "Nenhum item em estoque"} message={items.length ? "Ajuste os filtros para encontrar o item." : "Não há itens cadastrados nesta empresa."} /> : <>
        <div className="divide-y divide-[#0d1b2e]/8 md:hidden">
          {paged.map((item: any) => { const quantity = Number(item.quantity ?? 0); const minQuantity = Number(item.min_quantity ?? 0); return <article key={item.id} className="space-y-3 p-4">
        ...
        <div className="hidden overflow-x-auto md:block"><table className="min-w-[860px]"><thead><tr>{showName && <th className="text-left">Nome</th>}{showSku && <th className="text-left">SKU</th>}<th className="text-left">Endereço</th>{showQuantity && <th className="text-left">Quantidade</th>}{showMinQuantity && <th className="text-left">Mínimo</th>}{showPurchasePrice && <th className="text-left">Compra</th>}{showSalePrice && <th className="text-left">Venda</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>
          {paged.map((item: any) => <tr key={item.id} onClick={() => canViewDetails && setDetailItem(item)}>
            ...
            {showActions && <td onClick={event => event.stopPropagation()}><div className="flex justify-end gap-1.5">{canViewDetails && <AdminButton variant="secondary" size="sm" onClick={() => setDetailItem(item)}>Detalhes</AdminButton>}{canEdit && <AdminIconButton ariaLabel="Editar item" onClick={() => openEdit(item)}><Edit2 size={15} /></AdminIconButton>}{canMove && <AdminIconButton ariaLabel="Movimentar estoque" onClick={() => openMovement(item)}><ArrowLeftRight size={15} /></AdminIconButton>}{canViewMovements && <AdminIconButton ariaLabel="Histórico de movimentações" onClick={() => void openHistory(item)}><History size={15} /></AdminIconButton>}</div></td>}
          </tr>)}
        </tbody></table></div>
    ...
    <AdminDialog open={Boolean(detailItem)} onClose={() => setDetailItem(null)} title={detailItem?.name || "Detalhes do item"} description="Informações do estoque desta empresa." footer={detailItem && <div className="flex flex-wrap justify-end gap-2">{canToggle && <AdminButton variant="secondary" onClick={() => void toggleActive(detailItem)}>{detailItem.is_active !== false ? "Desativar" : "Ativar"}</AdminButton>}{canDelete && <AdminButton variant="danger" onClick={() => void removeItem(detailItem)}>Excluir</AdminButton>}{canEdit && <AdminButton onClick={() => { const item = detailItem; setDetailItem(null); openEdit(item); }}><Edit2 size={14} /> Editar</AdminButton>}</div>}>
    ...
    <AdminDialog open={creating || Boolean(editingItem)} onClose={closeItemEditor} title={editingItem ? "Editar item" : "Novo item"} description={editingItem ? "Atualize os dados do item desta empresa." : "Cadastre um novo item diretamente no estoque desta empresa."} footer={<div className="flex justify-end gap-2"><AdminButton variant="secondary" onClick={closeItemEditor}>Cancelar</AdminButton><AdminButton loading={savingItem} onClick={() => void saveItem()}>Salvar</AdminButton></div>}>
      <InventoryFormFields form={form} setForm={setForm} editing={Boolean(editingItem)} />
    </AdminDialog>

    <AdminDialog open={Boolean(movementItem)} onClose={() => setMovementItem(null)} title={movementItem ? `Movimentar · ${movementItem.name}` : "Movimentar estoque"} description={movementItem ? `Saldo atual: ${formatNumber(movementItem.quantity)} ${unitLabel(movementItem.unit)}.` : undefined} footer={<div className="flex justify-end gap-2"><AdminButton variant="secondary" onClick={() => setMovementItem(null)}>Cancelar</AdminButton><AdminButton loading={savingMovement} onClick={() => void saveMovement()}>Registrar</AdminButton></div>}>
      <div className="space-y-4"><FSelect label="Tipo" value={movementForm.type} options={[{ value: "in", label: "Entrada" }, { value: "out", label: "Saída" }, { value: "adjust", label: "Ajuste de saldo" }]} onChange={(e: any) => setMovementForm(current => ({ ...current, type: e.target.value as MovementForm["type"] }))} /><FIntegerInput label={movementForm.type === "adjust" ? "Novo saldo" : "Quantidade"} value={movementForm.quantity} onChange={(e: any) => setMovementForm(current => ({ ...current, quantity: e.target.value }))} /><FTextarea label="Motivo" value={movementForm.reason} onChange={(e: any) => setMovementForm(current => ({ ...current, reason: e.target.value }))} placeholder="Motivo da movimentação" /></div>
    </AdminDialog>
...
function InventoryFormFields({ form, setForm, editing }: { form: InventoryForm; setForm: Dispatch<SetStateAction<InventoryForm>>; editing: boolean }) {
  const patch = (key: keyof InventoryForm, value: any) => setForm(current => ({ ...current, [key]: value }));
  return <div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><FInput label="Nome" required value={form.name} onChange={(e: any) => patch("name", e.target.value)} /></div><FInput label="SKU" value={form.sku} onChange={(e: any) => patch("sku", e.target.value)} /><FSelect label="Unidade" value={form.unit} options={[{ value: "un", label: "Unidade" }, { value: "cx", label: "Caixa" }]} onChange={(e: any) => patch("unit", e.target.value as InventoryForm["unit"])} />{form.unit === "cx" && <FIntegerInput label="Unidades por caixa" value={form.conversion_factor} onChange={(e: any) => patch("conversion_factor", e.target.value)} />}{!editing && <FIntegerInput label="Quantidade inicial" value={form.quantity} onChange={(e: any) => patch("quantity", e.target.value)} />}<FIntegerInput label="Quantidade mínima" value={form.min_quantity} onChange={(e: any) => patch("min_quantity", e.target.value)} /><FCurrencyInput label="Preço de compra" value={form.purchase_price} onChange={(e: any) => patch("purchase_price", e.target.value)} /><FCurrencyInput label="Preço de venda" value={form.sale_price} onChange={(e: any) => patch("sale_price", e.target.value)} /><FInput label="Estante" value={form.storage_shelf} onChange={(e: any) => patch("storage_shelf", e.target.value)} /><FInput label="Prateleira" value={form.storage_level} onChange={(e: any) => patch("storage_level", e.target.value)} /><FInput label="Compartimento" value={form.storage_compartment} onChange={(e: any) => patch("storage_compartment", e.target.value)} /><div className="sm:col-span-2"><FTextarea label="Descrição" value={form.description} onChange={(e: any) => patch("description", e.target.value)} /></div><div className="sm:col-span-2"><FToggle label="Item ativo" description="Itens inativos ficam bloqueados para novas movimentações." checked={form.is_active} onChange={value => patch("is_active", value)} /></div></div>;
}
