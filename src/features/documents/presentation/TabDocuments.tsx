      ...
      {canViewTable && (section === "printing" ? <>
        ...
        <AdminCard>
          ...
          {attachmentTypesLoading ? <div className="p-8"><LoadingState /></div> : attachmentTypes.length === 0 ? <div className="p-8"><EmptyState icon={Tag} title="Nenhum tipo de anexo" /></div> : <>
            <div className="overflow-x-auto"><table className="min-w-[620px]"><thead><tr>{showAttachmentType && <th className="text-left">Tipo de anexo</th>}{showStatus && <th className="text-left">Status</th>}{showActions && <th className="text-right">Ações</th>}</tr></thead><tbody>{pagedAttachmentTypes.map(type => <tr key={type.id}>{showAttachmentType && <td className="font-bold text-[#0d1b2e]">{type.name}</td>}{showStatus && <td>{type.is_active ? "Ativo" : "Inativo"}</td>}{showActions && <td><div className="flex justify-end gap-1">{canEditAttachment && <><AdminButton variant="secondary" size="sm" onClick={() => toggleAttachmentType(type.id, !type.is_active)}>{type.is_active ? "Desativar" : "Ativar"}</AdminButton><AdminButton variant="secondary" size="sm" onClick={() => openTypeModal(type)}>Editar</AdminButton></>}{canDeleteAttachment && <AdminButton variant="danger" size="sm" onClick={() => deleteType(type)}>Excluir</AdminButton>}</div></td>}</tr>)}</tbody></table></div>
