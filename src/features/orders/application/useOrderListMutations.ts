...
export function useOrderListMutations({
  orders,
  setOrders,
  statuses,
  situations,
  ...
  const updateOrderStatus = async (order: any, statusId: string) => {
    if (!hasPermission("orders.status")) {
      showToast({
        msg: "Você não possui permissão para alterar o status.",
        type: "error",
      });
      return;
    }
    ...
    setOrders(current => current.map(item =>
      item.id === order.id
        ? { ...item, status_id: statusId, order_status: nextStatus || item.order_status }
        : item,
    ));
  ...
  const updateOrderSituation = async (order: any, situationId: string) => {
    if (!hasPermission("orders.edit")) {
      showToast({
        msg: "Você não possui permissão para alterar a situação.",
        type: "error",
      });
      return;
    }
  ...
  const handleKanbanDrop = async (statusId: string) => {
    ...
    try {
      ...
      const { error: historyError } = await insertServiceOrderStatusHistory(
        order.id,
        statusId,
        userId || null,
      );
      if (historyError) {
        console.warn(
          "[ADMIN] OS status history warning:",
      ...
      setOrders(previousOrders);
      showToast({
        msg: `Não foi possível alterar o status: ${error instanceof Error ? error.message : String(error)}`,
        type: "error",
      });
    } finally {
  ...
  const shouldSuppressCardOpen = () => {
    if (!suppressCardClickRef.current) return false;
    suppressCardClickRef.current = false;
    return true;
  };

  const handleDragLeave = (statusId: string) => {
