...
function PartCustodyWizard({ request, orderSolved, getCommittedQuantity }: {
  ...
  const destinationDone = purpose === "TEST" && deliveryDone && request.items.every(item =>
    ...
    Number(item.returned_quantity ?? 0) + Number(item.return_pending_quantity ?? 0) +
    Number(item.damaged_quantity ?? 0) + getCommittedQuantity(item),
  );
  ...
  return <div className="mt-3 min-w-0 overflow-hidden rounded-xl border border-[#0d1b2e]/10 bg-white p-3">
    ...
    <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{stages.map((stage, index) => <div key={`${stage.label}-${index}`} className={cn("min-w-0 rounded-lg border p-2.5", stage.state === "done" ? "border-emerald-200 bg-emerald-50" : stage.state === "current" ? "border-amber-300 bg-amber-50 ring-1 ring-amber-200" : stage.state === "error" ? "border-red-200 bg-red-50" : "border-[#0d1b2e]/8 bg-[#f8fafc] opacity-70")}><div className="flex min-w-0 items-start gap-2"><span className={cn("flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-black", stage.state === "done" ? "bg-emerald-600 text-white" : stage.state === "current" ? "bg-amber-500 text-white" : stage.state === "error" ? "bg-red-600 text-white" : "bg-[#dfe5ee] text-[#5a6a82]")}>{stage.state === "done" ? <Check size={12} /> : stage.state === "error" ? <X size={12} /> : index + 1}</span><div className="min-w-0"><p className={cn("break-words text-[11px] font-bold", stage.state === "error" ? "text-red-700" : "text-[#0d1b2e]")}>{stage.label}</p><p className="mt-0.5 break-words text-[10px] leading-snug text-[#5a6a82]">{stage.description}</p></div></div></div>)}</div>
  </div>;
}

export function OrderPartRequestsSection({ requests, assignedTo, currentUserId, orderSolved = false, hasPermission, formatDate, getCommittedQuantity, getPendingQuantity, onApprove, onReject, onCustody, onTestResult }: {
  ...
  onCustody: (request: PartRequestForReview, action: CustodyAction) => void;
  onTestResult: (request: PartRequestForReview) => void;
}) {
  return <Section title="Solicitações de peças">
    {requests.length === 0 ? <p className="text-xs text-[#5a6a82]">Nenhuma solicitação de peças para esta OS.</p> : <div className="min-w-0 space-y-3">{requests.map(request => {
      const status = String(request.status || "").toUpperCase();
      const purpose = request.purpose || "RESOLUTION";
      ...
      return <div key={request.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/10 bg-[#f8fafc] p-3">
        ...
        <div className="mt-2 min-w-0 space-y-2">{request.items.map(item => {
          ...
          return <div key={item.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/8 bg-white p-2.5"><p className="break-words text-xs font-semibold text-[#0d1b2e]">{item.inventory_item?.name || "Peça"}</p><p className="mt-0.5 break-words text-[11px] text-[#5a6a82]">Solicitado: {Number(item.quantity)} {unit}{item.approved_quantity != null && ` · Aprovado: ${Number(item.approved_quantity)} ${unit}`}</p>{item.source_test_item_id && <span className="mt-1 inline-flex max-w-full rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700">Origem: peça testada</span>}{delivered > 0 && <div className="mt-2 grid min-w-0 grid-cols-2 gap-1 text-[10px] sm:grid-cols-3 xl:grid-cols-7"><span className="min-w-0 rounded bg-blue-50 px-2 py-1 text-blue-700">Saída: {delivered} {unit}</span><span className="min-w-0 rounded bg-cyan-50 px-2 py-1 text-cyan-700">Recebida pelo técnico: {received} {unit}</span><span className="min-w-0 rounded bg-amber-50 px-2 py-1 text-amber-700">Devolução pendente: {returnPending} {unit}</span><span className="min-w-0 rounded bg-emerald-50 px-2 py-1 text-emerald-700">Recebida no estoque: {returned} {unit}</span><span className="min-w-0 rounded bg-red-50 px-2 py-1 text-red-700">Danificada: {damaged} {unit}</span>{purpose === "TEST" && <span className="min-w-0 rounded bg-violet-50 px-2 py-1 text-violet-700">Para resolução: {committed} {unit}</span>}{purpose === "TEST" && <span className="min-w-0 rounded bg-amber-50 px-2 py-1 text-amber-700">Aguardando: {pending} {unit}</span>}</div>}</div>;
        })}</div>
        ...
        <div className="mt-3 grid min-w-0 grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:justify-end">
          ...
          {hasPermission("orders.dispatch_parts") && status === "APPROVED" && hasPendingDispatch && <button type="button" onClick={() => onCustody(request, "DISPATCH")} className="min-w-0 rounded-lg bg-[#0057e7] px-3 py-2 text-xs font-bold text-white hover:bg-[#0046c0]">Confirmar saída</button>}
          {hasPermission("orders.confirm_part_delivery") && status === "APPROVED" && hasPendingDeliveryConfirmation && <button type="button" onClick={() => onCustody(request, "CONFIRM_DELIVERY")} className="min-w-0 rounded-lg bg-cyan-600 px-3 py-2 text-xs font-bold text-white hover:bg-cyan-700">Confirmar entrega ao técnico</button>}
          {hasPermission("orders.register_part_return") && status === "APPROVED" && hasReturnableItems && <button type="button" onClick={() => onCustody(request, "REGISTER_RETURN")} className="min-w-0 rounded-lg bg-amber-600 px-3 py-2 text-xs font-bold text-white hover:bg-amber-700">Registrar devolução</button>}
          {hasPermission("orders.receive_returned_parts") && status === "APPROVED" && hasPendingReturnReceipt && <button type="button" onClick={() => onCustody(request, "RECEIVE_RETURN")} className="min-w-0 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-700">Confirmar retorno ao estoque</button>}
          {status === "APPROVED" && purpose === "TEST" && hasDeliveredItems && hasPendingTestResult && currentUserId === assignedTo && <button type="button" onClick={() => onTestResult(request)} className="min-w-0 rounded-lg border border-[#0057e7]/30 px-3 py-2 text-xs font-bold text-[#0057e7]">Registrar resultado do teste</button>}
        </div>
      </div>;
    })}</div>}
  </Section>;
