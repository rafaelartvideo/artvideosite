                    ...
                    return <div key={request.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/10 bg-[#f8fafc] p-3">
                      ...
                      <div className="mt-2 min-w-0 space-y-2">{request.items.map(item => {
                        ...
                        return <div key={item.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/8 bg-white p-2.5">
                          ...
                          {normalizedPurpose === "TEST" && delivered > 0 && <div className="mt-2 grid min-w-0 grid-cols-2 gap-1 text-[10px] sm:grid-cols-5">
                            <span className="min-w-0 rounded bg-blue-50 px-2 py-1 text-blue-700">Entregue: {delivered} {unit}</span>
                            <span className="min-w-0 rounded bg-emerald-50 px-2 py-1 text-emerald-700">Devolvida: {returned} {unit}</span>
                            <span className="min-w-0 rounded bg-red-50 px-2 py-1 text-red-700">Danificada: {damaged} {unit}</span>
                            <span className="min-w-0 rounded bg-violet-50 px-2 py-1 text-violet-700">Para resolução: {committed} {unit}</span>
                            <span className="min-w-0 rounded bg-amber-50 px-2 py-1 text-amber-700">Aguardando: {pending} {unit}</span>
                          </div>}
                        </div>;
                      })}</div>
