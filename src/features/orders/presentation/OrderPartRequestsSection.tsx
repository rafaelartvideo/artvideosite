...
function PartCustodyWizard({
  ...
  const destinationDone = purpose === "TEST" && deliveryDone && request.items.every(item =>
  ...
  const stages = [
    ...
    step(
      ...
      rejected
        ? (status === "CANCELLED" ? "Solicitação cancelada" : "Pedido rejeitado pelo gestor")
        : approved ? "Quantidades aprovadas" : "Aguardando análise do gestor",
      rejected ? "error" : approved ? "done" : "current",
    ),
    step(
      "Saída do estoque",
      physicalItems.length === 0 && approved
        ? "Peça já estava com o técnico"
        : dispatchDone ? "Saída confirmada pelo estoquista" : approved ? "Aguardando confirmação do estoquista" : "Disponível após aprovação",
    ...
    ...(purpose === "TEST" ? [
      step(
        "Destino da peça",
        destinationDone
          ? (hasDamaged ? "Resultado e danos registrados" : "Resultado do teste registrado")
          : deliveryDone ? "Aguardando uso, devolução ou dano" : "Disponível após a entrega",
        destinationDone ? "done" as WizardStepState : deliveryDone ? "current" as WizardStepState : "pending" as WizardStepState,
      ),
      step(
        "Retorno ao estoque",
        hasReturnPending
          ? "Aguardando estoquista confirmar recebimento"
          : hasReturned ? "Recebimento confirmado e saldo reposto" : destinationDone ? "Sem devolução pendente" : "Disponível quando houver devolução",
        hasReturnPending ? "current" as WizardStepState : hasReturned || destinationDone ? "done" as WizardStepState : "pending" as WizardStepState,
      ),
    ] : [
      step(
        "Uso na resolução",
        orderSolved ? "Peça registrada na solução da OS" : deliveryDone ? "Disponível para resolver a OS" : "Disponível após a entrega",
        orderSolved ? "done" as WizardStepState : deliveryDone ? "current" as WizardStepState : "pending" as WizardStepState,
      ),
    ]),
  ];

  return (
    <div className="mt-3 min-w-0 overflow-hidden rounded-xl border border-[#0d1b2e]/10 bg-white p-3">
      <div className="mb-3 flex min-w-0 items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-wider text-[#0d1b2e]">Fluxo das peças</p>
          <p className="text-[10px] text-[#5a6a82]">{purpose === "TEST" ? "Pedido para teste" : "Pedido para resolução"}</p>
        </div>
        {stages.some(stage => stage.state === "current") && (
          <span className="shrink-0 rounded-full bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-700">Aguardando ação</span>
        )}
      </div>
        ...
        {stages.map((stage, index) => (
          <div
            ...
            className={cn(
              ...
              stage.state === "current" ? "border-amber-300 bg-amber-50 ring-1 ring-amber-200" :
              stage.state === "error" ? "border-red-200 bg-red-50" :
              "border-[#0d1b2e]/8 bg-[#f8fafc] opacity-70",
            )}
          >
            <div className="flex min-w-0 items-start gap-2">
              <span className={cn(
                ...
                stage.state === "current" ? "bg-amber-500 text-white" :
                stage.state === "error" ? "bg-red-600 text-white" :
                "bg-[#dfe5ee] text-[#5a6a82]",
              )}>
                {stage.state === "done" ? <Check size={12} /> : stage.state === "error" ? <X size={12} /> : index + 1}
              </span>
              <div className="min-w-0">
                <p className={cn("break-words text-[11px] font-bold", stage.state === "error" ? "text-red-700" : "text-[#0d1b2e]")}>{stage.label}</p>
                <p className="mt-0.5 break-words text-[10px] leading-snug text-[#5a6a82]">{stage.description}</p>
              </div>
            </div>
          </div>
        ))}
...
export function OrderPartRequestsSection({
  ...
  return (
    <Section title="Solicitações de peças">
      {detailPartRequests.length === 0 ? <p className="text-xs text-[#5a6a82]">Nenhuma solicitação de peças para esta OS.</p> : <div className="min-w-0 space-y-3">
        {detailPartRequests.map((request: PartRequestForReview) => {
          ...
          return <div key={request.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/10 bg-[#f8fafc] p-3">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <span className={cn("shrink-0 rounded-full px-2 py-1 text-[10px] font-bold", statusClass)}>{statusLabel}</span>
              <span className="shrink-0 rounded-full bg-blue-100 px-2 py-1 text-[10px] font-bold text-blue-700">{purposeLabel(normalizedPurpose)}</span>
              <span className="min-w-0 basis-full break-words text-[11px] text-[#5a6a82] sm:ml-auto sm:basis-auto sm:text-right">{request.requester?.full_name || "Solicitante não informado"} · {fmtDate(request.created_at, true)}</span>
            </div>
            {request.notes && <p className="mt-2 break-words whitespace-pre-line text-xs text-[#0d1b2e]">{request.notes}</p>}
            <PartCustodyWizard request={request} orderSolved={orderSolved} getCommittedQuantity={getTestCommittedQuantity} />
            <div className="mt-2 min-w-0 space-y-2">{request.items.map(item => {
              ...
              return <div key={item.id} className="min-w-0 overflow-hidden rounded-lg border border-[#0d1b2e]/8 bg-white p-2.5">
                ...
                {item.source_test_item_id && <span className="mt-1 inline-flex max-w-full rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700">Origem: peça testada</span>}
                {delivered > 0 && <div className="mt-2 grid min-w-0 grid-cols-2 gap-1 text-[10px] sm:grid-cols-3 xl:grid-cols-7">
                  <span className="min-w-0 rounded bg-blue-50 px-2 py-1 text-blue-700">Saída: {delivered} {unit}</span>
                  <span className="min-w-0 rounded bg-cyan-50 px-2 py-1 text-cyan-700">Recebida pelo técnico: {received} {unit}</span>
                  <span className="min-w-0 rounded bg-amber-50 px-2 py-1 text-amber-700">Devolução pendente: {returnPending} {unit}</span>
                  <span className="min-w-0 rounded bg-emerald-50 px-2 py-1 text-emerald-700">Recebida no estoque: {returned} {unit}</span>
                  <span className="min-w-0 rounded bg-red-50 px-2 py-1 text-red-700">Danificada: {damaged} {unit}</span>
                  {normalizedPurpose === "TEST" && <span className="min-w-0 rounded bg-violet-50 px-2 py-1 text-violet-700">Para resolução: {committed} {unit}</span>}
                  {normalizedPurpose === "TEST" && <span className="min-w-0 rounded bg-amber-50 px-2 py-1 text-amber-700">Aguardando: {pending} {unit}</span>}
                </div>}
              </div>;
            })}</div>
            ...
            <div className="mt-3 grid min-w-0 grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:justify-end">
              ...
              {hasPermission("orders.manage_part_requests") && status === "APPROVED" && normalizedPurpose === "TEST" && !hasDeliveredItems && <button type="button" onClick={() => onDelivery(request)} className="min-w-0 rounded-lg bg-[#0057e7] px-3 py-2 text-xs font-bold text-white hover:bg-[#0046c0]">Confirmar entrega</button>}
              {status === "APPROVED" && normalizedPurpose === "TEST" && hasDeliveredItems && hasPendingTestResult && user?.id === detail?.assigned_to && <button type="button" onClick={() => onTestResult(request)} className="min-w-0 rounded-lg border border-[#0057e7]/30 px-3 py-2 text-xs font-bold text-[#0057e7]">Registrar resultado do teste</button>}
            </div>
          </div>;
        })}
      </div>}
