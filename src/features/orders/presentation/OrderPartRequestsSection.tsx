...
export function OrderPartRequestsSection({ requests, assignedTo, currentUserId, orderSolved = false, hasPermission, formatDate, getCommittedQuantity, getPendingQuantity, onApprove, onReject, onDelivery, onTestResult }: {
  ...
  return <div className="min-w-0 space-y-4">{requests.map((request, requestIndex) => {
    ...
    return <AdminCard key={request.id} className="min-w-0 bg-white p-0">
      ...
      <div className="flex min-w-0 flex-wrap justify-end gap-2 border-t border-[#0d1b2e]/8 bg-[#f8fafc]/70 px-4 py-3 sm:px-5">
        {hasPermission("orders.manage_part_requests") && status === "PENDING" && <><button type="button" onClick={event => onApprove(event, request)} aria-label="Aprovar solicitação" title="Aprovar" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-0 text-xs font-semibold text-white hover:bg-emerald-700 sm:w-auto sm:px-3"><Check size={15} /><span className="hidden sm:inline">Aprovar</span></button><button type="button" onClick={event => onReject(event, request)} aria-label="Rejeitar solicitação" title="Rejeitar" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-red-600 px-0 text-xs font-semibold text-white hover:bg-red-700 sm:w-auto sm:px-3"><X size={15} /><span className="hidden sm:inline">Rejeitar</span></button></>}
        {hasPermission("orders.manage_part_requests") && status === "REJECTED" && <button type="button" onClick={event => onApprove(event, request)} aria-label="Aprovar solicitação" title="Aprovar" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-0 text-xs font-semibold text-white hover:bg-emerald-700 sm:w-auto sm:px-3"><Check size={15} /><span className="hidden sm:inline">Aprovar</span></button>}
        {hasPermission("orders.manage_part_requests") && status === "APPROVED" && !hasDeliveredItems && <button type="button" onClick={event => onReject(event, request)} aria-label="Desaprovar solicitação" title="Desaprovar" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-amber-600 px-0 text-xs font-semibold text-white hover:bg-amber-700 sm:w-auto sm:px-3"><RotateCcw size={15} /><span className="hidden sm:inline">Desaprovar</span></button>}
        {hasPermission("orders.dispatch_parts") && status === "APPROVED" && hasPendingDispatch && <button type="button" onClick={() => onDelivery(request, "DISPATCH")} aria-label="Confirmar saída" title="Confirmar saída" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-[#0057e7] px-0 text-xs font-semibold text-white hover:bg-[#0046c0] sm:w-auto sm:px-3"><Truck size={15} /><span className="hidden sm:inline">Confirmar saída</span></button>}
        {hasPermission("orders.confirm_part_delivery") && status === "APPROVED" && hasPendingDeliveryConfirmation && <button type="button" onClick={() => onDelivery(request, "CONFIRM_DELIVERY")} aria-label="Confirmar entrega ao técnico" title="Confirmar entrega ao técnico" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-cyan-600 px-0 text-xs font-semibold text-white hover:bg-cyan-700 sm:w-auto sm:px-3"><PackageCheck size={15} /><span className="hidden sm:inline">Confirmar entrega ao técnico</span></button>}
        {hasPermission("orders.register_part_return") && status === "APPROVED" && hasReturnableItems && <button type="button" onClick={() => onDelivery(request, "REGISTER_RETURN")} aria-label="Registrar devolução" title="Registrar devolução" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-amber-600 px-0 text-xs font-semibold text-white hover:bg-amber-700 sm:w-auto sm:px-3"><Undo2 size={15} /><span className="hidden sm:inline">Registrar devolução</span></button>}
        {hasPermission("orders.receive_returned_parts") && status === "APPROVED" && hasPendingReturnReceipt && <button type="button" onClick={() => onDelivery(request, "RECEIVE_RETURN")} aria-label="Confirmar retorno ao estoque" title="Confirmar retorno ao estoque" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-0 text-xs font-semibold text-white hover:bg-emerald-700 sm:w-auto sm:px-3"><PackageCheck size={15} /><span className="hidden sm:inline">Confirmar retorno ao estoque</span></button>}
        {hasPermission("orders.record_test_results") && status === "APPROVED" && purpose === "TEST" && hasDeliveredItems && hasPendingTestResult && currentUserId === assignedTo && <button type="button" onClick={() => onTestResult(request)} aria-label="Registrar resultado do teste" title="Registrar resultado do teste" className="inline-flex h-9 w-9 min-w-0 items-center justify-center gap-1.5 rounded-lg border border-[#0057e7]/30 bg-white px-0 text-xs font-semibold text-[#0057e7] hover:bg-[#eef5ff] sm:w-auto sm:px-3"><ClipboardCheck size={15} /><span className="hidden sm:inline">Registrar resultado do teste</span></button>}
      </div>
    </AdminCard>;
  })}</div>;
}
