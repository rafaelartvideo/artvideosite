...
export function OrderPartRequestsSection({ requests, assignedTo, currentUserId, orderSolved = false, hasPermission, formatDate, getCommittedQuantity, getPendingQuantity, onApprove, onReject, onDelivery, onTestResult }: {
  ...
  return <div className="min-w-0 space-y-4">{requests.map((request, requestIndex) => {
    ...
    return <AdminCard key={request.id} className="min-w-0 bg-white p-0">
      ...
      <div className="grid min-w-0 grid-cols-1 gap-2 border-t border-[#0d1b2e]/8 bg-[#f8fafc]/70 px-4 py-3 sm:flex sm:flex-wrap sm:justify-end sm:px-5">
        {hasPermission("orders.manage_part_requests") && status === "PENDING" && <><button type="button" onClick={event => onApprove(event, request)} className="min-w-0 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700">Aprovar</button><button type="button" onClick={event => onReject(event, request)} className="min-w-0 rounded-lg bg-red-600 px-3 py-2 text-xs font-semibold text-white hover:bg-red-700">Rejeitar</button></>}
        {hasPermission("orders.manage_part_requests") && status === "REJECTED" && <button type="button" onClick={event => onApprove(event, request)} className="min-w-0 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700">Aprovar</button>}
        {hasPermission("orders.manage_part_requests") && status === "APPROVED" && !hasDeliveredItems && <button type="button" onClick={event => onReject(event, request)} className="min-w-0 rounded-lg bg-amber-600 px-3 py-2 text-xs font-semibold text-white hover:bg-amber-700">Desaprovar</button>}
        {hasPermission("orders.dispatch_parts") && status === "APPROVED" && hasPendingDispatch && <button type="button" onClick={() => onDelivery(request, "DISPATCH")} className="min-w-0 rounded-lg bg-[#0057e7] px-3 py-2 text-xs font-semibold text-white hover:bg-[#0046c0]">Confirmar saída</button>}
        {hasPermission("orders.confirm_part_delivery") && status === "APPROVED" && hasPendingDeliveryConfirmation && <button type="button" onClick={() => onDelivery(request, "CONFIRM_DELIVERY")} className="min-w-0 rounded-lg bg-cyan-600 px-3 py-2 text-xs font-semibold text-white hover:bg-cyan-700">Confirmar entrega ao técnico</button>}
        {hasPermission("orders.register_part_return") && status === "APPROVED" && hasReturnableItems && <button type="button" onClick={() => onDelivery(request, "REGISTER_RETURN")} className="min-w-0 rounded-lg bg-amber-600 px-3 py-2 text-xs font-semibold text-white hover:bg-amber-700">Registrar devolução</button>}
        {hasPermission("orders.receive_returned_parts") && status === "APPROVED" && hasPendingReturnReceipt && <button type="button" onClick={() => onDelivery(request, "RECEIVE_RETURN")} className="min-w-0 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-700">Confirmar retorno ao estoque</button>}
        {hasPermission("orders.record_test_results") && status === "APPROVED" && purpose === "TEST" && hasDeliveredItems && hasPendingTestResult && currentUserId === assignedTo && <button type="button" onClick={() => onTestResult(request)} className="min-w-0 rounded-lg border border-[#0057e7]/30 bg-white px-3 py-2 text-xs font-semibold text-[#0057e7] hover:bg-[#eef5ff]">Registrar resultado do teste</button>}
      </div>
    </AdminCard>;
  })}</div>;
}
