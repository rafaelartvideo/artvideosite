...
const packageHint = (item: { package_unit?: string | null; conversion_factor?: number | null }) =>
  item.package_unit === "cx" && Number(item.conversion_factor || 1) > 1
    ? `Embalagem: caixa · ${Number(item.conversion_factor)} un/cx`
    : "";
      ...
      <div className="space-y-5">
        <AdminCard className="p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <AdminCard className={cn("shadow-none transition-all", purpose === "RESOLUTION" ? "border-[#0057e7] bg-[#f0f6ff] ring-1 ring-[#0057e7]/10" : "hover:border-[#0057e7]/35 hover:bg-[#f8fbff]")}><button type="button" onClick={() => onPurposeChange("RESOLUTION")} aria-pressed={purpose === "RESOLUTION"} className="group w-full p-4 text-left"><span className="flex items-center gap-2 text-sm font-black text-[#0d1b2e]"><span className={cn("flex h-8 w-8 items-center justify-center rounded-lg", purpose === "RESOLUTION" ? "bg-[#0057e7] text-white" : "bg-[#f5f7fa] text-[#5a6a82]")}><CheckCircle size={16} /></span>Para resolução</span><span className="mt-2 block text-xs leading-relaxed text-[#5a6a82]">Peças destinadas diretamente à solução definitiva da ordem de serviço.</span></button></AdminCard>
            <AdminCard className={cn("shadow-none transition-all", purpose === "TEST" ? "border-[#0057e7] bg-[#f0f6ff] ring-1 ring-[#0057e7]/10" : "hover:border-[#0057e7]/35 hover:bg-[#f8fbff]")}><button type="button" onClick={() => onPurposeChange("TEST")} aria-pressed={purpose === "TEST"} className="group w-full p-4 text-left"><span className="flex items-center gap-2 text-sm font-black text-[#0d1b2e]"><span className={cn("flex h-8 w-8 items-center justify-center rounded-lg", purpose === "TEST" ? "bg-[#0057e7] text-white" : "bg-[#f5f7fa] text-[#5a6a82]")}><PackagePlus size={16} /></span>Para teste</span><span className="mt-2 block text-xs leading-relaxed text-[#5a6a82]">Peças retiradas temporariamente para diagnóstico, teste e posterior definição do destino.</span></button></AdminCard>
          </div>
        </AdminCard>

        <div className="min-w-0 space-y-5">
          <AdminCard className="min-w-0">
            <AdminCardHeader className="block p-4">
              <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-sm font-black text-[#0d1b2e]">Estoque</p><p className="mt-0.5 text-[11px] text-[#5a6a82]">O saldo abaixo já está convertido para unidades individuais.</p></div><span className="shrink-0 rounded-full bg-[#f5f7fa] px-2 py-1 text-[10px] font-bold text-[#5a6a82]">{visibleItems.length} item{visibleItems.length === 1 ? "" : "s"}</span></div>
              <div className="relative mt-3"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#5a6a82]" /><input autoFocus value={search} onChange={event => onSearchChange(event.target.value)} placeholder="Buscar por nome ou SKU" className={cn(INPUT, "h-11 w-full pl-9 pr-3 text-sm")} /></div>
            </AdminCardHeader>
            <div className="max-h-[19rem] min-h-[10rem] overflow-y-auto p-3">
              {inventoryLoading ? <div className="flex min-h-36 items-center justify-center text-xs text-[#5a6a82]">Carregando peças do estoque...</div> : inventoryError ? <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs font-semibold text-red-700">{inventoryError}</div> : visibleItems.length === 0 ? <div className="flex min-h-36 flex-col items-center justify-center rounded-xl border border-dashed border-[#0d1b2e]/10 text-center"><Search size={20} className="mb-2 text-[#9aa7b8]" /><p className="text-xs font-bold text-[#5a6a82]">Nenhuma peça encontrada</p></div> : <div className="space-y-2">{visibleItems.map(item => {
                ...
                return <AdminCard key={item.id} className={cn("shadow-none transition-colors", selected ? "border-[#0057e7]/20 bg-[#f0f6ff]" : "hover:bg-[#f8fafc]")}><div className="flex min-w-0 items-center gap-3 p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-[#0d1b2e]">{item.name}</p><div className="mt-1 flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-[#5a6a82]">{item.sku && <span className="truncate">SKU {item.sku}</span>}<span className="font-semibold text-[#0d1b2e]">Disponível: {Number(item.quantity)} un</span>{hint && <span>{hint}</span>}</div></div><button type="button" disabled={selected || Number(item.quantity) <= 0} onClick={() => onSelect(item)} className={cn("inline-flex h-9 shrink-0 items-center justify-center rounded-lg px-3 text-xs font-black transition-colors", selected ? "bg-emerald-100 text-emerald-700" : "border border-[#0057e7]/25 bg-white text-[#0057e7] hover:bg-[#f0f6ff]", Number(item.quantity) <= 0 && "cursor-not-allowed opacity-50")}>{selected ? "Adicionada" : "Adicionar"}</button></div></AdminCard>;
              })}</div>}
            </div>
          </AdminCard>

          <AdminCard className="min-w-0">
            <AdminCardHeader><div className="min-w-0"><p className="text-sm font-black text-[#0d1b2e]">Peças selecionadas</p><p className="mt-0.5 text-[11px] text-[#5a6a82]">Informe quantas unidades individuais são necessárias.</p></div><span className="shrink-0 rounded-full bg-[#0057e7] px-2.5 py-1 text-[10px] font-black text-white">{selectedItems.length}</span></AdminCardHeader>
            <div className="max-h-[19rem] min-h-[10rem] overflow-y-auto p-3">
              {selectedItems.length === 0 ? <div className="flex min-h-36 flex-col items-center justify-center rounded-xl border border-dashed border-[#0d1b2e]/10 px-4 text-center"><PackagePlus size={22} className="mb-2 text-[#9aa7b8]" /><p className="text-xs font-bold text-[#5a6a82]">Nenhuma peça selecionada</p><p className="mt-1 text-[11px] text-[#8a97a8]">Adicione itens do estoque para montar a solicitação.</p></div> : <div className="space-y-2">{selectedItems.map(item => <AdminCard key={item.inventory_item_id} className="min-w-0 bg-[#f8fafc] p-3 shadow-none"><div className="flex min-w-0 items-start gap-3"><div className="min-w-0 flex-1"><p className="break-words text-sm font-bold leading-snug text-[#0d1b2e]">{item.name}</p><p className="mt-1 break-words text-[11px] text-[#5a6a82]">{item.sku ? `SKU ${item.sku} · ` : ""}Disponível: {item.available_quantity} un</p></div><AdminIconButton ariaLabel={`Remover ${item.name}`} onClick={() => onRemove(item.inventory_item_id)} variant="danger" className="h-9 w-9 shrink-0"><X size={15} /></AdminIconButton></div><div className="mt-3 space-y-2"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wider text-[#5a6a82]">Quantidade (un)</p><p className="mt-1 text-[11px] text-[#8a97a8]">Máximo {item.available_quantity} un</p></div><input type="number" min="1" max={item.available_quantity} step="1" value={item.quantity} onChange={event => onQuantityChange(item.inventory_item_id, event.target.value)} inputMode="numeric" aria-label={`Quantidade de ${item.name}`} className={cn(INPUT, "h-10 w-full text-center text-sm font-bold sm:max-w-44")} /></div></AdminCard>)}</div>}
            </div>
          </AdminCard>
        </div>

        <AdminCard className="p-4"><FTextarea label="Observações" value={notes} onChange={(event: any) => onNotesChange(event.target.value)} rows={3} placeholder="Adicione informações importantes para quem fará a análise do pedido." /></AdminCard>
      </div>
    </div>
...
export function PartCustodyModal({ request, action, quantities, submitting, onQuantitiesChange, onClose, onSubmit }: {
  request: PartRequestForReview; action: CustodyAction; quantities: Record<string, string>; submitting: boolean; onQuantitiesChange: (value: Record<string, string>) => void; onClose: () => void; onSubmit: () => void;
}) {
