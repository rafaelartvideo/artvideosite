...
import { AdminCard, AdminCardHeader, BtnSecondary } from "@/shared/ui/admin/AdminLayout";
...
export function OrderSituationRecordsPage({
  ...
  const pageContext = useContext(AdminPageContext);
  ...
  useEffect(() => {
    ...
    return () => {
      pageContext.setPage(current => current?.onBack === stableClose ? previousPage : current);
    };
  }, [open, order?.os_number, pageContext?.setPage, stableClose]);

  useEffect(() => {
    if (!open) return;
    const handleKeyDown = (event: KeyboardEvent) => event.key === "Escape" && stableClose();
  ...
  return <div className="absolute inset-0 z-[80] min-h-full overflow-y-auto bg-[#f8fafc]">
    <div className="mx-auto w-full min-w-0 max-w-6xl p-4 sm:p-6 lg:p-8">
    ...
    <div
      className="fixed inset-x-0 z-[90] border-t border-[#0d1b2e]/10 bg-white/95 px-3 pt-3 shadow-[0_-10px_30px_rgba(13,27,46,0.10)] backdrop-blur md:sticky md:bottom-0 md:z-auto md:bg-white md:px-5 md:py-4 md:shadow-none md:backdrop-blur-none"
      style={{ bottom: browserBottomInset ? `${browserBottomInset}px` : 0, paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}
    >
      <div className="mx-auto w-full max-w-6xl"><BtnSecondary onClick={stableClose} className="w-full justify-center md:w-auto">Voltar para a OS</BtnSecondary></div>
    </div>
  </div>;
}
