...
export function OrdersKanban({ statuses, filteredOrders, situations, draggingId, hasPermission, onOpen, onSituationChange, onEdit, formatDate }: {
  ...
  return <div className="pb-3 md:overflow-x-auto"><div className="grid min-w-0 grid-cols-1 items-start gap-3 md:flex md:min-w-max md:gap-4">{statuses.map(status => {
    ...
    return <div key={status.id} className="w-full min-w-0 overflow-hidden rounded-xl border border-[#0d1b2e]/8 bg-[#f8fafc] md:w-[300px] md:flex-shrink-0">
      ...
      <div className="min-h-0 space-y-2 p-2.5 md:min-h-[180px] md:space-y-3 md:p-3">{statusOrders.length === 0 ? <p className="py-5 text-center text-[11px] text-[#5a6a82] md:py-10 md:text-xs">Nenhuma OS.</p> : statusOrders.map(order => <div key={order.id} draggable={false} onClick={() => canOpenDetails && onOpen(order)} className={cn("cursor-default rounded-lg border border-[#0d1b2e]/10 bg-white p-3 shadow-sm transition-colors", canOpenDetails && "hover:border-[#0057e7]/30", draggingId === order.id && "opacity-50")}>
        ...
        {canEdit && !order.is_solved && !isCancelled(order) && <div className="mt-3 flex items-center justify-end" onClick={event => event.stopPropagation()}><button type="button" onClick={() => onEdit(order)} className="flex min-h-9 cursor-default items-center gap-1 rounded-lg border border-[#0057e7]/30 px-3 py-1.5 text-xs font-bold text-[#0057e7]"><Edit2 size={13} /> Editar</button></div>}
      </div>)}</div>
    </div>;
  })}</div></div>;
}
