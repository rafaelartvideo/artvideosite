    ...
    <div className="fixed inset-x-0 z-[70] border-t border-[#0d1b2e]/10 bg-white/95 px-3 pt-3 shadow-[0_-10px_30px_rgba(13,27,46,0.10)] backdrop-blur md:sticky md:bottom-0 md:z-auto md:bg-white md:px-5 md:py-4 md:shadow-none md:backdrop-blur-none" style={{ bottom: browserBottomInset ? `${browserBottomInset}px` : 0, paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom, 0px))" }}>
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-2.5 md:flex-row md:items-center md:justify-between md:gap-3">
        ...
        {(canRequestParts || canResolve || canComplete || canEditOrder || canCancel) && <div className="flex min-w-0 flex-wrap justify-end gap-1.5 md:gap-2">
          {canRequestParts && <AdminButton variant="secondary" onClick={onRequestParts} aria-label="Pedir peças" title="Pedir peças" className="h-11 w-11 min-w-0 px-0 md:h-10 md:w-auto md:px-4"><PackagePlus className="h-5 w-5 md:h-4 md:w-4" /><span className="sr-only md:not-sr-only">Pedir peças</span></AdminButton>}
          {canResolve && <BtnPrimary onClick={onResolve} className="h-11 w-11 min-w-0 px-0 md:h-10 md:w-auto md:px-4"><Wrench className="h-5 w-5 md:h-4 md:w-4" /><span className="sr-only md:not-sr-only">Solucionar</span></BtnPrimary>}
          {canComplete && <BtnPrimary onClick={() => onComplete()} className="h-11 w-11 min-w-0 px-0 md:h-10 md:w-auto md:px-4"><CheckCircle className="h-5 w-5 md:h-4 md:w-4" /><span className="sr-only md:not-sr-only">Concluir OS</span></BtnPrimary>}
          {canEditOrder && <BtnPrimary onClick={onEdit} className="h-11 w-11 min-w-0 px-0 md:h-10 md:w-auto md:px-4"><Pencil className="h-5 w-5 md:h-4 md:w-4" /><span className="sr-only md:not-sr-only">Editar OS</span></BtnPrimary>}
          {canCancel && <AdminButton variant="secondary" onClick={() => setCancelOpen(true)} className="h-11 w-11 min-w-0 border-red-200 px-0 text-red-600 hover:bg-red-50 md:h-10 md:w-auto md:px-4" aria-label="Cancelar OS" title="Cancelar OS"><Ban className="h-5 w-5 md:h-4 md:w-4" /><span className="sr-only md:not-sr-only">Cancelar</span></AdminButton>}
        </div>}
      </div>
    </div>
    <OrderCancelDialog
