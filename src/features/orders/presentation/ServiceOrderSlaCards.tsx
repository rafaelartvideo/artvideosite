...
export function ServiceOrderSlaCards({ order, slaHours }: { order: any; slaHours: number | null }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">
    ...
    <div className="rounded-xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-4 shadow-sm"><div className="flex items-start gap-3"><div className="rounded-lg bg-blue-100 p-2 text-[#0057e7]"><Clock size={18} /></div><div><p className="text-[10px] font-black uppercase tracking-wider text-[#0057e7]">Tempo total da ordem de serviço</p><p className="mt-1 text-2xl font-black text-[#0d1b2e]">{formatElapsedHours(orderElapsed)}</p><p className="text-[11px] text-[#5a6a82]">{order.completed_at ? "Tempo encerrado na conclusão" : "Correndo desde a abertura da OS"}</p></div></div><div className="mt-3 flex items-center justify-between rounded-lg border border-blue-100 bg-white/70 px-3 py-2 text-[11px]"><span className="font-semibold text-[#5a6a82]">Início</span><span className="font-bold text-[#0d1b2e]">{new Date(order.created_at).toLocaleString("pt-BR")}</span></div></div>
  </div>;
}
