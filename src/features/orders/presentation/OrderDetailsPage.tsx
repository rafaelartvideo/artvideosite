...
export function OrderDetailsPage(props: Props) {
  ...
  return <>
      ...
      <OrderHistoryPage
        ...
        onClose={onCloseSubpage}
      />
      {partRequestsPageOpen && detail && (
        <AdminPage open onClose={onCloseSubpage} breadcrumb={`Ordens de Serviço > ${detail.os_number || "OS"} > Solicitações de peças`} title="Solicitações de peças" subtitle="Acompanhe os pedidos e o fluxo das peças desta OS" maxW="max-w-2xl">
          <div className="p-5">
      ...
      {visible && !routedSubpageOpen && (
        <AdminPage open onClose={closePage} breadcrumb="Ordens de Serviço" title={detail.os_number || "Ordem de Serviço"} subtitle={(detail.service as any)?.title || "Ordem de Serviço"} maxW="max-w-2xl">
