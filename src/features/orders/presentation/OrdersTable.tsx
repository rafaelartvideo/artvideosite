...
export function OrdersTable({ loading, filteredOrders, pagedOrders, totalItems, hasActiveFilters, hasPermission, onOpen, onSituationChange, getSituations, onEdit, onComplete, onCancel, cancellingId, formatDate, equipmentSummary, page, pageSize, totalPages, onPageChange, onPageSizeChange }: {
  ...
  const canComplete = (order: any) => canCompleteOrder && Boolean(order.is_solved) && !order.completed_at && !isCancelled(order);
    ...
    <AdminCard>
      {loading ? <div className="flex min-h-[300px] flex-col items-center justify-center gap-3 px-4 py-12 text-center" role="status" aria-live="polite" aria-busy="true"><LoadingSpinner size="lg" /><p className="text-sm font-semibold text-[#5a6a82]">Carregando ordens...</p></div> : filtered.length === 0 ? <EmptyState icon={ClipboardList} title="Nenhuma OS encontrada" message={hasActiveFilters ? "Tente ajustar os filtros." : "Crie a primeira OS com o botão Nova OS."} /> : <>
        <div className="divide-y divide-[#0d1b2e]/8 md:hidden">{pagedOrders.map(o => <article key={o.id} role={canOpenDetails ? "button" : undefined} tabIndex={canOpenDetails ? 0 : undefined} onClick={() => openDetails(o)} onKeyDown={event => { if (canOpenDetails && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); openDetails(o); } }} className="min-w-0 cursor-default space-y-3 overflow-hidden p-4 transition-colors active:bg-[#f5f7fa]">
          <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><OrderStatusDot status={(o.order_status as any)?.name} color={(o.order_status as any)?.color} /><div className="min-w-0">{hasPermission("orders.table.protocol") && <p className="font-mono text-sm font-black text-[#0057e7]">OS {o.os_number || "—"}</p>}{hasPermission("orders.table.customer") && <p className="truncate text-sm font-bold text-[#0d1b2e]">{(o.customer as any)?.full_name || "—"}</p>}</div></div></div>{hasPermission("orders.table.priority") && <div className="shrink-0"><PriorityBadge priority={o.priority || "normal"} /></div>}</div>
          ...
          {hasPermission("orders.table.actions") && <div onClick={event => event.stopPropagation()} className="flex min-w-0 flex-wrap items-center gap-1.5 border-t border-[#0d1b2e]/8 pt-3">
            ...
            {canComplete(o) && <AdminButton variant="secondary" size="sm" onClick={event => { event.stopPropagation(); onComplete(o); }} aria-label={`Concluir OS ${o.os_number || ""}`} title="Concluir OS" className="h-9 shrink-0 border-green-200 px-2.5 text-green-700 hover:bg-green-50"><CheckCircle2 size={15} /><span>Concluir</span></AdminButton>}
        ...
        <div className="hidden overflow-x-auto md:block"><table className="min-w-[1100px]"><thead><tr><th className="w-28 text-left">Protocolo</th><th className="text-left">Cliente</th><th className="w-40 text-left">Tipo de atendimento</th><th className="w-40 text-left">Equipamento</th><th className="w-24 text-left">Prioridade</th><th className="w-32 text-left">Data de agendamento</th><th className="w-28 text-left">Status</th><th className="w-36 text-left">Situação</th><th className="w-36 text-right">Ações</th></tr></thead><tbody>{pagedOrders.map(o => <tr key={o.id} onClick={() => openDetails(o)} className="cursor-default">
          {hasPermission("orders.table.protocol") && <td><div className="flex items-center gap-2"><OrderStatusDot status={(o.order_status as any)?.name} color={(o.order_status as any)?.color} /><span className="font-mono text-xs font-black text-[#0057e7]">{o.os_number || "—"}</span></div></td>}
          ...
          {hasPermission("orders.table.actions") && <td><div className="flex items-center justify-end gap-2">
            ...
            {canComplete(o) && <AdminButton variant="secondary" size="sm" onClick={event => { event.stopPropagation(); onComplete(o); }} className="border-green-200 text-green-700 hover:bg-green-50"><CheckCircle2 size={14} /> Concluir</AdminButton>}
