...
import { Ban, CheckCircle2, ClipboardList, Edit2 } from "lucide-react";
...
export function OrdersTable({ loading, filteredOrders, pagedOrders, totalItems, hasActiveFilters, hasPermission, onOpen, onSituationChange, getSituations, onEdit, onComplete, onCancel, cancellingId, formatDate, equipmentSummary, page, pageSize, totalPages, onPageChange, onPageSizeChange }: {
  ...
  onEdit: (order: any) => void; onComplete: (order: any) => void; onCancel: (order: any, reason: string) => Promise<boolean>; cancellingId: string | null;
    ...
    <AdminCard>
      {loading ? <div className="flex min-h-[300px] flex-col items-center justify-center gap-3 px-4 py-12 text-center" role="status" aria-live="polite" aria-busy="true"><LoadingSpinner size="lg" /><p className="text-sm font-semibold text-[#5a6a82]">Carregando ordens...</p></div> : filtered.length === 0 ? <EmptyState icon={ClipboardList} title="Nenhuma OS encontrada" message={hasActiveFilters ? "Tente ajustar os filtros." : "Crie a primeira OS com o botão Nova OS."} /> : <>
        <div className="divide-y divide-[#0d1b2e]/8 md:hidden">{pagedOrders.map(o => <article key={o.id} role={canOpenDetails ? "button" : undefined} tabIndex={canOpenDetails ? 0 : undefined} onClick={() => openDetails(o)} onKeyDown={event => { if (canOpenDetails && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); openDetails(o); } }} className="min-w-0 cursor-default space-y-3 overflow-hidden p-4 transition-colors active:bg-[#f5f7fa]">
          <div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-2"><span aria-label={`Cor do status ${(o.order_status as any)?.name || "Sem status"}`} className="h-8 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: (o.order_status as any)?.color || "transparent" }} /><div className="min-w-0">{hasPermission("orders.table.os") && <div className="flex min-w-0 flex-wrap items-center gap-1.5"><p className="font-mono text-sm font-black text-[#0057e7]">OS {o.os_number || "—"}</p>{isSolvedPendingCompletion(o) && <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold uppercase text-green-700">Solucionada</span>}</div>}{hasPermission("orders.table.external_os") && o.external_os_number && <p className="truncate text-xs font-bold text-[#5a6a82]">OS Externa {o.external_os_number}</p>}{hasPermission("orders.table.customer") && <p className="truncate text-sm font-bold text-[#0d1b2e]">{(o.customer as any)?.full_name || "—"}</p>}</div></div></div>{hasPermission("orders.table.priority") && <div className="shrink-0"><PriorityBadge priority={o.priority || "normal"} /></div>}</div>
          <div className="flex min-w-0 flex-wrap items-center gap-1.5">{hasPermission("orders.table.status") && <StatusBadge status={(o.order_status as any)?.name || "—"} color={(o.order_status as any)?.color} />}{hasPermission("orders.table.situation") && (o.situation as any)?.name && <StatusBadge status={(o.situation as any).name} color={(o.situation as any)?.color} />}{o.cannot_be_solved && !isCancelled(o) && <span className="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">⚠ Não solucionável</span>}</div>
          ...
          {hasPermission("orders.table.actions") && <div onClick={event => event.stopPropagation()} className="flex min-w-0 flex-wrap items-center gap-1.5 border-t border-[#0d1b2e]/8 pt-3">
            ...
            {canCancel(o) && <AdminButton variant="secondary" size="sm" onClick={event => { event.stopPropagation(); setCancelTarget(o); }} aria-label={`Cancelar OS ${o.os_number || ""}`} title="Cancelar OS" className="h-9 w-9 min-w-0 shrink-0 border-red-200 p-0 text-red-600 hover:bg-red-50"><Ban size={15} /></AdminButton>}
          </div>}
        </article>)}</div>
        <div className="hidden overflow-x-auto md:block"><table className="min-w-[1200px]"><thead><tr>{hasPermission("orders.table.os") && <th className="w-36 text-left">OS</th>}{hasPermission("orders.table.external_os") && <th className="w-36 text-left">OS Externa</th>}{hasPermission("orders.table.customer") && <th className="text-left">Cliente</th>}{hasPermission("orders.table.service_type") && <th className="w-40 text-left">Tipo de atendimento</th>}{hasPermission("orders.table.equipment") && <th className="w-40 text-left">Equipamento</th>}{hasPermission("orders.table.priority") && <th className="w-24 text-left">Prioridade</th>}{hasPermission("orders.table.scheduled_at") && <th className="w-32 text-left">Data de agendamento</th>}{hasPermission("orders.table.status") && <th className="w-28 text-left">Status</th>}{hasPermission("orders.table.situation") && <th className="w-36 text-left">Situação</th>}{hasPermission("orders.table.actions") && <th className="w-36 text-right">Ações</th>}</tr></thead><tbody>{pagedOrders.map(o => <tr key={o.id} onClick={() => openDetails(o)} className="cursor-default">
          {hasPermission("orders.table.os") && <td><div className="flex items-center gap-2"><span aria-label={`Cor do status ${(o.order_status as any)?.name || "Sem status"}`} className="h-8 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: (o.order_status as any)?.color || "transparent" }} /><div className="flex min-w-0 flex-wrap items-center gap-1.5"><span className="font-mono text-xs font-black text-[#0057e7]">{o.os_number || "—"}</span>{isSolvedPendingCompletion(o) && <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold uppercase text-green-700">Solucionada</span>}</div></div></td>}
          ...
          {hasPermission("orders.table.actions") && <td><div className="flex items-center justify-end gap-2">
            ...
            {canCancel(o) && <AdminButton variant="secondary" size="sm" onClick={event => { event.stopPropagation(); setCancelTarget(o); }} className="border-red-200 text-red-600 hover:bg-red-50"><Ban size={14} /> Cancelar</AdminButton>}
          </div></td>}
        </tr>)}</tbody></table></div>
      </>}
