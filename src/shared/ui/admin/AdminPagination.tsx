...
export function PaginationBar({
  ...
  useEffect(() => {
    ...
    if (pageSize !== defaultPageSize) onPageSizeChange(defaultPageSize);
  ...
  return <div className={`flex min-w-0 flex-col gap-3 border-t border-[#0d1b2e]/8 bg-[#f8fafc] px-4 py-3 sm:flex-row sm:items-center ${showPageSizeSelector ? "sm:justify-between" : "sm:justify-end"}`}>
    {showPageSizeSelector && <div className="flex min-w-0 items-center justify-between gap-2 text-xs text-[#5a6a82] sm:justify-start">
      <span>Linhas:</span>
      <div className="w-[88px]"><AdminSelect value={pageSize} onValueChange={value => onPageSizeChange(Number(value))} options={pageSizeOptions.map(value => ({ value, label: String(value) }))} className="min-h-11 py-2 text-xs sm:min-h-8 sm:py-1.5" ariaLabel="Linhas por página" /></div>
    </div>}
