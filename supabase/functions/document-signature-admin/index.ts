...
const SUMMARY_SELECT = [
  ...
  "require_employee_signature",
  "external_signer_type",
  "external_signer_name",
  "external_signer_email",
  "external_signer_phone",
  "external_document_masked",
  "employee_entity_id",
...
function snapshotHtml(snapshot: any) {
  ...
  const sectionHtml = sections.map((section: any) => {
    const fields = (Array.isArray(section.fields) ? section.fields : []).map((field: any) => {
  ...
  const checklistHtml = checklists.map((stage: any) => {
    const rows = (Array.isArray(stage.items) ? stage.items : []).map((item: any) => `<tr><td>${escapeHtml(item.title)}</td><td>${escapeHtml(item.response_text || item.response_code || item.response_number ?? "")}</td><td>${escapeHtml(item.observation || "")}</td></tr>`).join("");
...
function sanitizeSections(raw: unknown, allowedFields: Set<string>) {
  ...
  })).filter((section: any) => section.key && section.fields.length > 0);
}

function sanitizeChecklists(raw: unknown) {
  if (!Array.isArray(raw)) return [];
  return raw.slice(0, 30).map((stage: any) => ({
    ...
    items: (Array.isArray(stage?.items) ? stage.items : []).slice(0, 300).map((item: any) => ({
      ...
      media: (Array.isArray(item?.media) ? item.media : []).slice(0, 20).map((media: any) => ({
...
async function loadTemplateFieldKeys(adminClient: any, organizationId: string, templateId: string) {
  ...
  const { data: fields, error: fieldError } = await adminClient
    ...
    .select("field_key,is_enabled,sort_order,template_section_id")
...
async function entityIdForProfile(adminClient: any, organizationId: string, profileId: string | null) {
  ...
  for (const detail of details || []) {
    const { data: entity } = await adminClient.from("entities").select("id").eq("id", detail.entity_id).eq("organization_id", organizationId).maybeSingle();
...
async function resolveExternalSigner(adminClient: any, organizationId: string, order: any, template: any, input: any) {
  ...
  if (type !== "contact") throw new Error("Selecione quem assinará o documento.");
...
async function sendInviteEmail({ recipient, signerName, documentName, orderNumber, expiresAt, link, companyName }: any) {
  ...
  if (!resendApiKey || !emailFrom) throw new Error("Configure RESEND_API_KEY e DOCUMENT_SIGNATURE_EMAIL_FROM/ORDER_DOCUMENT_EMAIL_FROM na Edge Function.");
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: emailFrom,
      to: [recipient],
      subject: `${documentName} — assinatura eletrônica — OS ${orderNumber}`,
      html: `<div style="font-family:Arial,sans-serif;color:#172536;line-height:1.55"><h2 style="margin:0 0 12px">${escapeHtml(companyName)}</h2><p>Olá, ${escapeHtml(signerName)}.</p><p>Você recebeu o documento <strong>${escapeHtml(documentName)}</strong>, referente à OS <strong>${escapeHtml(orderNumber)}</strong>, para assinatura eletrônica.</p><p><a href="${escapeHtml(link)}" style="display:inline-block;background:#0057e7;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700">Abrir documento para assinatura</a></p><p style="font-size:12px;color:#526174">Este link expira em ${escapeHtml(expiration)}. Para sua segurança, o documento só será exibido após confirmação de identidade e código enviado por e-mail.</p></div>`,
    }),
  });
...
async function recordEvent(adminClient: any, request: Request, row: { id: string; organization_id: string }, eventType: string, actorType: string, actorUserId: string | null, metadata: Record<string, unknown> = {}) {
...
async function createAction({ request, body, user, authClient, adminClient, tokenKey, identityPepper }: any) {
  ...
  const allowedFields = new Set(currentFields);
  const safeSnapshot = {
    schema_version: 1,
    template: {
      id: template.id,
    ...
    sections: sanitizeSections(rawSnapshot.sections, allowedFields),
    checklists: sanitizeChecklists(rawSnapshot.checklists),
  };
  const canonicalSnapshot = canonicalStringify(safeSnapshot);
  if (new TextEncoder().encode(canonicalSnapshot).byteLength > MAX_SNAPSHOT_BYTES) throw new Error("O documento excede o tamanho permitido para assinatura online.");
  ...
  const expiresAt = new Date(Date.now() + Math.max(1, Math.min(720, Number(template.signature_link_ttl_hours) || 72)) * 60 * 60 * 1000).toISOString();
  ...
  try {
    const htmlBlob = new Blob([snapshotHtml(safeSnapshot)], { type: "text/html; charset=utf-8" });
    const { error: snapshotUploadError } = await adminClient.storage.from("signed-documents").upload(snapshotPath, htmlBlob, { contentType: "text/html; charset=utf-8", upsert: false, cacheControl: "3600" });
    ...
    if (employeeSignature && employeeSignaturePath) {
      const { data: signatureBlob, error: downloadError } = await adminClient.storage.from("employee-signatures").download(employeeSignature.storage_path);
      if (downloadError || !signatureBlob) throw downloadError || new Error("Não foi possível congelar a assinatura do funcionário.");
      const { error: copyError } = await adminClient.storage.from("signed-documents").upload(employeeSignaturePath, signatureBlob, { contentType: "image/png", upsert: false, cacheControl: "3600" });
    ...
    const requestRow = {
      ...
      created_by: user.id,
      supersedes_request_id: previous?.id || null,
    ...
    const { data: inserted, error: insertError } = await adminClient.from("document_signature_requests").insert(requestRow).select(SUMMARY_SELECT).single();
    ...
    if (employeeSignature && employeeEntity && employeeSignaturePath) {
      ...
      const { error: employeeEvidenceError } = await adminClient.from("document_signatures").insert({
        organization_id: organizationId,
      ...
      if (employeeEvidenceError) console.error("[DOCUMENT SIGNATURE EMPLOYEE EVIDENCE]", employeeEvidenceError.message);
    }
    ...
    if (uploadedPaths.length) await adminClient.storage.from("signed-documents").remove(uploadedPaths).catch(() => undefined);
    throw error;
  }
}

async function listAction({ request, body, user, authClient, adminClient }: any) {
  const organizationId = requireUuid(body.organization_id, "Empresa");
  const serviceOrderId = requireUuid(body.service_order_id, "OS");
...
async function linkAction({ request, body, user, authClient, adminClient, tokenKey }: any) {
  const organizationId = requireUuid(body.organization_id, "Empresa");
...
async function resendEmailAction({ request, body, user, authClient, adminClient, tokenKey }: any) {
  const organizationId = requireUuid(body.organization_id, "Empresa");
  const requestId = requireUuid(body.request_id, "Solicitação");
  ...
  const sentAt = new Date().toISOString();
  const { error } = await adminClient.from("document_signature_requests").update({ last_email_sent_at: sentAt }).eq("id", row.id).eq("organization_id", organizationId);
  if (error) throw error;
...
async function cancelAction({ request, body, user, authClient, adminClient }: any) {
  const organizationId = requireUuid(body.organization_id, "Empresa");
  const requestId = requireUuid(body.request_id, "Solicitação");
...
async function auditAction({ body, user, authClient, adminClient }: any) {
  const organizationId = requireUuid(body.organization_id, "Empresa");
  const requestId = requireUuid(body.request_id, "Solicitação");
