...
create or replace function public.list_financial_counterparties(
...
begin
  ...
  select
    e.id,
    coalesce(nullif(btrim(e.trade_name), ''), nullif(btrim(e.name), ''), nullif(btrim(e.legal_name), ''), 'Cadastro') as name,
    nullif(btrim(e.document), '') as document,
    coalesce(array_agg(er.role order by er.role) filter (where er.is_active = true), array[]::text[]) as roles
  from public.entities e
  left join public.entity_roles er on er.entity_id = e.id
  where e.organization_id = p_organization_id
    and e.is_active = true
  group by e.id, e.trade_name, e.name, e.legal_name, e.document;
end;
$$;

revoke all on function public.list_financial_counterparties(uuid, text) from public, anon;
grant execute on function public.list_financial_counterparties(uuid, text) to authenticated;
...
begin
  ...
  if v_is_new then
    ...
    values(p_organization_id,v_entry_type,v_description,v_issue_date,v_competence_date,v_original_amount,'pending',v_required_approvals,v_counterpart_id,v_counterpart_name,v_counterpart_document,'manual',v_notes,v_user_id,v_user_id)
